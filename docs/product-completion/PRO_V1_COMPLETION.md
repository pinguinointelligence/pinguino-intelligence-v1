...
implementation proceeds slice-by-slice. **Delivered & deployed:** Phase 0, S1 (`2a65849`,
capability data + tests), S2 (`0f7816f`), S3 (`b715222`).

**Under the completion rule, neither S2 nor S3 is CLOSED — both await the owner's real
authenticated staging test (the local DEV persona is NOT equivalent to a Pro login):**
- **S2 — `DEPLOYED, AWAITING OWNER AUTHENTICATED VERIFICATION`.** Code + gates + direct-DB
  RLS/immutability proofs done; the persistence round-trip (login → save → refresh → v2 →
  restore v1→v3 → logout/login → data remains) is unrun by me and owner-only.
- **S3 — `DEPLOYED, AWAITING OWNER AUTHENTICATED AND RESPONSIVE VERIFICATION`.** Built +
  browser-checked with the DEV persona only; needs verification on the served staging app with
  a real Pro entitlement.

S4 is **prepared but NOT started** — held until the owner returns the S2 + S3 staging results.

### Deployment baseline (verified 2026-07-21, direct)
| Ref | Commit |
|---|---|
| local HEAD | `7440b83` |
| origin/main | `7440b83` |
| origin/staging | `7440b83` |
| staging deployed (staging.pinguinoai.com, dpl_J9JgR…, target production, READY) | `7440b83` |
| production deployed (www.pinguinoai.com, dpl_AbWuQT6…, target production, READY) | `7440b83` |
| staging bundle | `assets/index-BYZ-Y9Xz.js` — inlines `tunabqqrwabacxjcxxkz.supabase.co` |
| production bundle | `assets/index-Bd_4A_26.js` — **inlines NO supabase.co URL** (PI-P0-001) |

**Currently served commit = `7440b83` on both.** `7440b83` = S3 code (`b715222`) **+ one
docs-only commit** (`git diff --stat b715222 7440b83` = only `PRO_V1_COMPLETION.md`), so the
running application bundle is exactly the S3 code. Prod carries the S3 *code* but, lacking the
Supabase env (PI-P0-001), **cannot authenticate a Pro login** — so S2/S3 verification can only
happen on **staging**.
...
- **S2 — Pro-core Supabase adapters + selector wiring** — DEPLOYED, AWAITING OWNER AUTHENTICATED VERIFICATION (`0f7816f`): recipes/versions (0027), production (0028), costs (0029). Existing domains persist for real; see the S2 ledger below.
- **S3 — `/pro` workspace shell + persona-gated nav** — DEPLOYED, AWAITING OWNER AUTHENTICATED & RESPONSIVE VERIFICATION (`b715222`): 9-tab canonical Pro workspace (Receptura = the reused engine lab via extracted `StudioEngineSurface`; Wersje = real S2-backed `RecipeVersionsSection`; Produkcja/Koszty = live durable-backend indicator + honest "arrives in S_" note; Ustawienia/Maszyna wired). Non-Pro → honest upsell. `/studio` kept intact (demo/free previews unchanged) + cross-links to `/pro`; the `/studio`→`/pro` redirect deferred to a later slice to avoid regressing that preview. Browser-verified on the dev server.
...
**Status: `DEPLOYED, AWAITING OWNER AUTHENTICATED VERIFICATION` — NOT CLOSED.** Code + gates +
direct-DB RLS/immutability proofs are done and no S2 code defect remains, but per the completion
rule the slice is not CLOSED until the owner runs the real authenticated staging round-trip
(login → create → save → hard refresh → reopen → v2 → restore v1→v3 → logout/login → data
remains). The local DEV persona is not equivalent. Production runtime parity: `BLOCKED_EXTERNAL`
(B1 / PI-P0-001) — unchanged.
