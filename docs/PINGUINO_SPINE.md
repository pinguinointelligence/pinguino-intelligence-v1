...
and the **solver target override for a real gram solve** (Phase C Slice 14 — additive optional
`targetBandOverride` on the engine solver; the preview runs engine-seeded vs regulator-shadow gram solves,
global `TARGET_BANDS` unchanged, default behavior byte-identical)
...
- **NOT STARTED (the Spine's Recipe-Intelligence behavior):** PRODUCTION (non-DEV) Studio surfacing of
  the optimization preview + persistence, batch-size/actual-batch-rescue user flow, stock-shortage flow,
  User Flow conversational script. (Recipe Intent normalization, the Designer's `RecipeDesignPlan`, the
...
solves side by side. Not in production Studio yet.)
