...
and the **Stock Shortage branch IF10** (Phase C Slice 18 — pure `stockShortageRouter`, shortage observation
→ substitution/scale/purchase/reformulation decision with never-silent substitution safety gates and the
locked §18 user-decision menu; see [STOCK_SHORTAGE_FLOW.md](spine/STOCK_SHORTAGE_FLOW.md))
...
- **NOT STARTED (the Spine's Recipe-Intelligence behavior):** the LIVE accepted-correction write path
  ...
  but no migration is applied and nothing writes), the IF9/IF10 USER flows + exact recalculation
  (Slices 17–18 built the pure DECISION branches — `batchRescueRouter` + `stockShortageRouter`,
  unwired, gram-free), User Flow conversational script. (PRODUCTION (non-DEV) Studio SURFACING of the optimization preview is
