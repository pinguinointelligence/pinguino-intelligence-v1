...
and the **Actual Batch Rescue branch IF9** (Phase C Slice 17 — pure `batchRescueRouter`, observed-problem
...
- **NOT STARTED (the Spine's Recipe-Intelligence behavior):** the LIVE accepted-correction write path
  ...
  but no migration is applied and nothing writes), the actual-batch-rescue USER flow + exact gram solve
  (Slice 17 built the pure IF9 DECISION branch — `batchRescueRouter`, unwired, gram-free),
  stock-shortage flow (IF10), User Flow conversational script. (PRODUCTION (non-DEV) Studio SURFACING of the optimization preview is
