...
global `TARGET_BANDS` unchanged, default behavior byte-identical) and the **production Studio optimization
...
- **NOT STARTED (the Spine's Recipe-Intelligence behavior):** optimization-preview PERSISTENCE
  (accepted-correction save), batch-size/actual-batch-rescue user flow, stock-shortage flow,
...
dev-only. Persistence of accepted corrections is the remaining step.)
