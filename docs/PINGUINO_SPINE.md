...
Demo redaction-safe, save-vs-apply explicit; NO conversational assistant yet, NO persistence —
see [STUDIO_USER_FLOW_PL.md](studio/STUDIO_USER_FLOW_PL.md)) added 2026-07-10. This is the repo-level companion to the **locked Spine
...
| User Flow | **Partial** (PL-first guidance layer live; conversational script not started) | `src/features/studioFlow/` (2026-07-10): locked PL copy registry (10 situations + tier/save/disclaimer overlays, honesty rules test-pinned — never "zapisano"/"zastosowano", partial ≠ rescued, Demo redaction-safe, save-vs-apply explicit) + pure `studioFlowGuidance` mapper + read-only `StudioFlowGuidePanel` in Studio; tone per the locked [User_Flow.md](pinguino-spine/User_Flow.md) §14; `src/features/pi-chat/` deterministic intake still exists (English, different script) | Medium (conversational shell + preference persistence pending) | conversational question-script shell consuming this registry (locked §3–§7 order, flavor-first, never price-first); preference defaults persistence is a future owner-approved DB slice |
