...
and the **solver-injected regulator targets** preview (Phase C Slice 13 — `solverTargetInjection`,
...
comparison** (Slice 12) audits the engine-vs-regulator band gap, and a **solver-injected regulator target**
preview (Slice 13) re-targets the solver's violation detection at the regulator bands to show what would
change — both WITHOUT altering live engine config, `calculateRecipe` or the gram solver.
Not in production Studio yet.)
