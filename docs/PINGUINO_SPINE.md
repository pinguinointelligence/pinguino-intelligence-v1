...
the **IF9 multi-step add-only rescue walk** (Phase C Slice 20 — `batchRescueStepSolver`,
smallest-verified-fraction stepping via the Slice-14 override, regulator-verified per step,
direction-guarded; the −12 too-hard fixture becomes an honest `partial_improvement`) the
**production IF9/IF10 branch UI** (Phase C Slice 21 — `BranchWorkflowPreviews` in Studio: paid-gated,
explicit-click, local non-persisted measurement forms, redacted panel, zero apply/save controls) the
**IF10 verified-composition substitute contract** (Phase C Slice 22 — `verifiedSubstituteContract` +
`previewVerifiedSubstituteRecalculation`: allowlisted provenance with Mapper/PI-Calculated explicitly
denied, full safety gates, engine+regulator-verified split-swap; production UI never accepts typed-in
compositions) the **IF9 multi-lever rescue expansion** (Phase C Slice 23 —
`batchRescueMultiLeverSolver`: residual-gate candidate search across lever families, best-verified
selection, unified `calculated` = regulator-accepts semantics; measured full two-lever rescue)
and the **accepted-correction LIVE write path** (Phase C Slice 24 — migration
`0012_accepted_corrections` applied under locked owner decisions A–I: separate immutable audit
table, owner-scoped RLS with NO update policy/grant proven by transaction-scoped negative tests;
`src/services/acceptedCorrections.ts` create/list/delete with the Slice-16 draft as its input gate;
`SaveCorrectionControl` in Studio, signed-in Pro only, explicit click, honest record-id success;
Mapper/products/PAC-POD/statuses untouched; end-to-end signed-in save proven and cleaned up in
Slice 24B — see
[ACCEPTED_CORRECTION_PERSISTENCE_PLAN.md](spine/ACCEPTED_CORRECTION_PERSISTENCE_PLAN.md) §8)
and the **accepted-correction server-side tier enforcement** (audit verdict
`server_tier_source_ready` on the client-unwritable `subscriptions` cache; **Option A APPLIED as
migration `0013`** — the INSERT policy now requires an active/trialing/past_due-in-grace
subscription AND an own-recipe link, proven by a 14-case transaction-scoped RLS matrix plus a
signed-in owner save/delete under the new policy; Option-B Edge Function source remains NOT
deployed — plan §9/§9.1) added 2026-07-10, and the **temperature-aware engine TARGET_BANDS**
(owner-approved ENGINE slice, CONFIG 0.5.0 → **0.6.0**: all 12 locked profile × temperature cells
seeded verbatim from the regulator docs — milk −12/−13, chocolate/sorbet/vegan × −11/−12/−13;
the DEFAULT classifier + solver now target the recipe's real cell; sorbet/vegan omit the
regulator-disabled dairy gates (`TargetBand.metrics` now Partial); chocolate protein-share uses
the locked hard-min 7; goldens re-baselined with every diff explained (7 of 8 = version stamp
only; chocolate-classic = the intended band flip); the Slice 12–14 shadow/injection seams now
report ALIGNED/no-op for seeded cells; −11°C Engine Contract rev 1A.6 — see
[TEMPERATURE_AWARE_TARGET_BANDS_PLAN.md §7](engine/TEMPERATURE_AWARE_TARGET_BANDS_PLAN.md))
added 2026-07-10, and the **PL-first Studio User-Flow guidance layer**
(`src/features/studioFlow/` — locked PL copy registry + pure state→copy mapper + read-only
`StudioFlowGuidePanel` in the Studio right rail; 10 situations + tier/save/disclaimer overlays;
honesty rules test-pinned: never "zapisano"/"zastosowano", partial improvement never a rescue,
Demo redaction-safe, save-vs-apply explicit; NO persistence —
see [STUDIO_USER_FLOW_PL.md](studio/STUDIO_USER_FLOW_PL.md)) added 2026-07-10, and the **PL-first
deterministic Conversational Assistant Shell** (`src/features/studioFlow/conversationalAssistantFlow.ts`
+ `StudioAssistantShell.tsx` — a fixed 10-question PL state machine feeding the locked
`normalizeRecipeIntent` into a read-only `AssistantIntentDraft`; NO LLM, NO persistence, NO recipe
mutation, NO grams generation; branch context recipe_design/actual_batch_rescue/stock_shortage;
honest "szkic" copy — never a recipe-created claim; see
[STUDIO_ASSISTANT_FLOW_PL.md](studio/STUDIO_ASSISTANT_FLOW_PL.md)) added 2026-07-10, and the
**intent → deterministic starter recipe draft** (`intentRecipeDraft.ts` — maps an
`AssistantIntentDraft` to a LOCAL read-only starter recipe from LOCKED base templates built ONLY
from the production demo/reference catalog: standard_gelato + chocolate_gelato ready,
sorbet/vegan honestly `not_supported`, never faked; runs the real `calculateRecipe` for a preview
with honest inBand/optimization-recommended — never an "optimized" claim; exact grams Pro-gated;
NO LLM/DB/persistence/recipe mutation; see
[INTENT_TO_RECIPE_DRAFT.md](studio/INTENT_TO_RECIPE_DRAFT.md)) added 2026-07-10, and the
**Spine decision-layer acceptance matrix + dairy candidate gate** (Phase C Slice 25 — the full
12-cell profile × temperature grid, preview≡engine equality, batch-total coherence and
fresh-evaluation invariants pinned in `spineDecisionAcceptance.test.ts`; PLUS the one defect found:
the engine correction-candidate catalog offered DAIRY candidates (smp/cream/milk) to sorbet/vegan
exact solves — a too-soft vegan batch came back `calculated` with "add Milk 3.5 % ≈691 g" — fixed
by category-gating the three dairy candidates and appending the category-gated `water` lever to
`npac_high`/`ice_fraction_low`, dairy-category solves byte-identical (test-pinned); see
[SPINE_DECISION_ACCEPTANCE.md](spine/SPINE_DECISION_ACCEPTANCE.md)) added 2026-07-11. This is the repo-level companion to the **locked Spine
...
- **NOT STARTED (the Spine's Recipe-Intelligence behavior):** the LIVE accepted-correction write path
  ...
  but no migration is applied and nothing writes), branch APPLY/SAVE (Slices 17–21 built the pure
  IF9/IF10 decision branches, the dispatch, the verification-gated exact-recalculation preview with
  multi-step rescue, and the PAID-GATED production Studio preview UI — but nothing applies or persists;
  a user decision from the locked menus still executes nothing), User Flow conversational script. (PRODUCTION (non-DEV) Studio SURFACING of the optimization preview is
...
| Recipe Engine (Base Engine) | **Partial** (core Done at −11 °C; target bands temperature-aware) | deterministic pure core, ENGINE 0.4.0 / **CONFIG 0.6.0** (`src/engine/config/version.ts`), golden recipes re-baselined (7/8 version-stamp-only; chocolate-classic = intended band flip), no-NPAC regression; `src/engine/**`. CONFIG 0.6.0 (2026-07-10, owner-approved): `TARGET_BANDS` seeds all 12 locked regulator cells — the DEFAULT classifier/solver target the recipe's real profile×temperature band; sorbet/vegan omit disabled dairy gates; chocolate protein-share hard-min 7; fruit/nut/alcohol categories keep the documented milk fallback; ice ANCHORS still −11-calibrated (bands live, anchors are the next calibration frontier) | Low — data-only config change, one-commit rollback | ice anchors / external recompute fixtures for −12/−13 when calibration data arrives; regulator configs — never duplicate engines |
...
| User Flow | **Partial** (PL guidance layer + deterministic assistant shell live; LLM + persistence not started) | `src/features/studioFlow/` (2026-07-10): (a) locked PL copy registry (10 situations + tier/save/disclaimer overlays) + pure `studioFlowGuidance` mapper + read-only `StudioFlowGuidePanel`; (b) the deterministic **Conversational Assistant Shell** — `conversationalAssistantFlow.ts` (fixed 10-question PL state machine → `normalizeRecipeIntent` → read-only `AssistantIntentDraft`, branch context recipe_design/actual_batch_rescue/stock_shortage) + `StudioAssistantShell.tsx`; (c) **intent → deterministic starter recipe draft** — `intentRecipeDraft.ts` (locked base templates from the demo/reference catalog; standard+chocolate ready, sorbet/vegan not_supported; real `calculateRecipe` preview, honest inBand/optimization-recommended; exact grams Pro-gated). NO LLM, NO persistence, NO recipe mutation, honest "szkic" copy; tone per the locked [User_Flow.md](pinguino-spine/User_Flow.md); `src/features/pi-chat/` intake still exists (English) | Medium (LLM/richer parsing + preference persistence + local apply pending) | local apply-to-Studio-draft; preference defaults persistence (owner-approved DB slice); richer Designer rules (sorbet/vegan templates, flavor→ingredient); optional LLM assistant over the same registry |
...
- **Exists:** `src/engine/corrections/` — candidates, exact-gram solving, apply, verification by
  ...
  Since Slice 25 (2026-07-11) the candidate catalog is CATEGORY-GATED for the dairy rule: dairy
  candidates (smp/cream/milk) never reach sorbet/vegan solves; water is the sorbet/vegan/fruit
  dilution lever for `npac_high`/`ice_fraction_low`; dairy-category behavior byte-identical
  (test-pinned — see [SPINE_DECISION_ACCEPTANCE.md](spine/SPINE_DECISION_ACCEPTANCE.md) §6).
- **Missing vs Spine:** fuller profile-specific candidate families (e.g. a calibrated plant-fat
  candidate for vegan `fat_low` — honestly `no_valid_correction` today),
