...
`OptimizationPreviewPanel` + capability/redaction policy, DEV-gated into Studio) added 2026-07-08. This
is the repo-level companion to the **locked Spine
...
- **NOT STARTED (the Spine's Recipe-Intelligence behavior):** PRODUCTION (non-DEV) Studio surfacing of
  ...
  Slices 2–8; a **DEV preview** at `/dev/optimization-preview` (Slice 9) and a reusable
  **OptimizationPreviewPanel** + capability/redaction policy wired **DEV-gated** into the LIVE Studio
  recipe (Slice 10) followed — real solver + `calculateRecipe`, nothing saved. Not in production Studio yet.)
