...
and the **accepted-correction server-side tier enforcement hardening** (proposal-stage:
`server_tier_source_ready` audit of the client-unwritable `subscriptions` cache; RECOMMENDED
Option-A tier-checking INSERT policy as a NON-applied proposal + Option-B Edge Function
`create-accepted-correction` as NON-deployed source with lockstep contract tests; tier remains
client/service-enforced until the owner approves one — plan §9) added 2026-07-10. This is the repo-level companion to the **locked Spine
