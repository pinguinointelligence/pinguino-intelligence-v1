...
`src/features/optimization/`) added 2026-07-08. This is the repo-level companion to the **locked Spine
...
- **NOT STARTED (the Spine's Recipe-Intelligence behavior):** production Studio wiring of the
  solver-rerun preview, batch-size/actual-batch-rescue user flow, stock-shortage flow, User Flow
  conversational script. (Recipe Intent normalization, the Designer's `RecipeDesignPlan`, the
  ...
  Slices 2–8; a **DEV preview** at `/dev/optimization-preview` now wires the seam to the REAL solver +
  `calculateRecipe` via a non-spine orchestrator — Slice 9. Not in production Studio yet.)
