...
5. **OCR / image intake** — ❌ still missing (OCR itself). Keyless/local only (e.g. browser Tesseract.js); `incomplete_text` red flag already guards partial OCR. **NEW** pure `intakeClassifier.ts` ROUTES an input by filename/text (csv/xlsx → table import · image → OCR-pending · EAN → keyless lookup · else unknown) with a live panel on `/dev/intake-hub` — routing only, no OCR performed, no fetch.
