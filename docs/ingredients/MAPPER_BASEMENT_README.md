...
The active Mapper 2089 source has:

- 2,089 unique `ingredient_id` rows and 62 columns;
- stable IDs from `PI-ING-000001` through owner-authorized `PI-ING-002114`, with intentional gaps;
...
- The replacement seed runs in one transaction, soft-deactivates rows absent from the active source, upserts and reactivates the 2,089 canonical rows, and asserts the final active count.
...
A reviewed product may be promoted only by a human-approved replacement of the active CSV and regenerated seed. `PI-ING-002114` is the single owner-authorized 2026-08-28 expansion; there is no automatic promotion or mutation path.
