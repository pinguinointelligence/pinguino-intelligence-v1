...
| PC-03 | HIGH | **CLOSED / FROZEN — owner-approved served QA 2026-08-31.** The 22 recorded cells are harness artefacts (`buildInput` appends rotation ingredients without budgeting them; the Sorbet extra bypasses the stabilizer clamp). Neither citrus fibre nor NEAREST coverage was causal. Real defect: Sorbet exact-projection eligibility required the incoming draft to already be on batch, though the projection solves FOR the target batch — so off-batch drafts took the ~50–92 s general path and at ±30 g published an Engine violation. Fixed narrowly: off batch **+ exactly one Main**, batch restored first. staging `c464075e` · GEL-P0-025. | 22 |
