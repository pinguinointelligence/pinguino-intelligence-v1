...
| H-55-1 | Actions | Every ingredient has a `⋯` menu: Remove, Find substitute where supported, "I don't have this ingredient" | Y (Pro) | `createSubstitutionPreview`, `markIngredientUnavailable` | Y | `/` | `constraintStudioStore` | – | – | Y | Y | Y | Y | TESTED | Shared compatibility-first `ProductPickerPopover` is opened manually from HOME Base and Topping row menus; it excludes the exact current identity, preserves role/stage, and fails closed with `Brak zgodnych zamienników`. Automated evidence: `HOME-REPLACE-01`, `REPLACE-COMPAT-01`, `REPLACE-ROLE-01`, `REPLACE-EMPTY-01`, `RSX-01`, `RSX-02`, `RSX-03`, `RPL-BASE-01`, `RPL-BASE-02`, `RPL-BASE-03`, `RPL-TOP-01`, `RPL-TOP-02` in the focused 152/152 pass. **Corrective I (2026-09-12):** the mutable „Wszystkie” control had cleared the line-derived cream context and widened the complete list before segmentation. The complete replacement candidate set is now hard-gated by immutable scope/role/family/form compatibility before recency or ranking. `RSX-04` and `REPLACE-COMPAT-02` prove Cream retains only cream-compatible rows and excludes Banana Fabbri Cream and Amaretto Cream while keeping compatible recents. Served QA remains pending. | |
| H-55-2 | Actions | Base and flavour ingredients are not functionally separated | – | – | Y | `/` | – | – | – | Y | Y | Y | Y | TODO | | |
| H-56-1 | Actions | `Add ingredient` reuses the SAME Pro picker (search, filters, voice, scanner, catalogue, ProductBehavior) with a simpler HOME presentation | Y | `product-picker` | Y | `/` | `services/productPicker` | – | – | Y | Y | Y | Y | TESTED | HOME Add and PRO Search use the shared picker/catalog boundary and final `planMapperCatalogSearch` resolver contract; HOME automatically selects central legal row #1 while PRO remains manual. Automated evidence: `HOME-ADD-01`, `HOME-ADD-02`, `PRO-BANANA-01`, `PROFILE-01`, `PRO-FILTER-01`, shared discovery/live-search contracts in the focused 152/152 pass. **Corrective G (2026-09-12):** central search selected exact banana `PI-ING-000345`, but the first loss was the next authenticated-only rich Mapper hydration on anonymous HOME. An exact-id-only immutable toolbox materializer now preserves that selected identity and hands it to HOME's existing amount confirmation; it never searches, ranks or guesses. `HOME-G-01` proves central #1 survives permission denial and the identical Mapper id/name reaches the recipe. Served QA remains pending. | |
