...
| E-REV-04 | Partner copy | The rest of `/partner` reads as copy too — two raw statuses remain: profile moderation (`APPROVED` / `UNDER_REVIEW` / `DISABLED`, Profile section) and partner status (`active` / `suspended` / `terminated`, Settings) | ⚪ | ⬜ | ⬜ | ⬜ | 🔓 | — | Found during D-LINK-03, which fixed the code and link statuses. E-REV-03's guard lists only `String(row.*)` renders, so neither was caught | Display map + extend the guard, as in E-REV-03 |
