...
| Q-ASSET-13 | Assets | **Owner's final 13 assets imported, unmodified** | 🟢 | ✅ | ✅ | ⬜ | 🔓 | `248bbfd9` | A01–A07, F01, F03, W01–W04 in `public/images/work-with-us/`, **md5-verified byte-identical** to the owner's files. WebP delivery derivatives at 1600/800px (27 MB → 1.7 MB); originals kept as the authority. No crop, no filter, no logo touched | — |
| Q-ASSET-14 | Assets | Manifest ↔ delivered-file reconciliation | 🟢 | ✅ | ⬜ | ⬜ | 🔓 | `ownerAssets.ts` | **3 MISMATCHES recorded, not smoothed over**: A04 (manifest "Machines hero" — no machine in frame), A06 (manifest "writer persona 4:3" — is a trailer at 16:9), W01 (manifest "/machines hero" — is an equipment close-up) | Owner confirms the re-mapping |
| Q-ASSET-15 | Assets | **`/machines` has no hero asset** | 🔴 | ⬜ | ⬜ | ⬜ | 🔓 | — | Genuinely missing slot, not a missing filename: A04 and W01 were the manifest's two Machines candidates and **neither features a machine**. W01's pozzetti counter can carry an equipment detail card, but the lane has no product-led hero | **OWNER_DECISION_REQUIRED** — supply a Machines hero, or approve a lane without one |
| Q-ASSET-16 | Assets | Cart model identity is unresolved | ⚪ | ⬜ | ⬜ | ⬜ | 🔓 | — | A05 and W02 show two different carts; the canonical set is Battery Cart · V2C · V4C. Nothing in the images identifies which is which, so they are placed by **use case** and carry no model label | **OWNER_DECISION_REQUIRED** |
| Q-ASSET-17 | Assets | Two different trailers delivered | ⚪ | ⬜ | ⬜ | ⬜ | 🔓 | — | A06/A07 are a single-axle white/graphite trailer; W03 is a **twin-axle aluminium** one. Both are Gellatti trailers; they are not presented as the same product | Owner confirms whether both are on-sale formats |
