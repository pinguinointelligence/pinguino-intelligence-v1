...
| C-APP-08 | Notifications | Emails: received · more info requested · approved · rejected · payout setup required · payout/account requirements | 🟡 | ✅ | ⬜ | ⬜ | 🔓 | — | **UNBLOCKED by owner correction §3** — Resend adapter behind a provider-agnostic `EmailProvider` port. Depends on EMAIL-01..06 | Build the email lane first · **READY / WAITING OWNER DB APPROVAL.** Migration `20260910180000` + rollback written, NOT applied (shared prod DB). Trigger on the status TRANSITION so all writers are covered without reproducing either large RPC. Uses only the canonical `gellatti_enqueue_email_v1` and the five taxonomy subjects that already existed and had never been used. draft/under_review/suspended/terminated deliberately unmailed — no taxonomy subject exists and the taxonomy is closed on purpose. 13 contract tests. LIVE MAIL QA DEFERRED: no isolated mail sink |
