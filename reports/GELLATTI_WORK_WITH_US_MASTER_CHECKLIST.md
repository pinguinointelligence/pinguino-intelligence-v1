...
| H-DASH-07 | Dashboard | Commissions list: product · cadence · amount · earned · eligible · status · reversal · anonymized customer ref | 🟡 | ✅ | ⬜ | ⬜ | 🔓 | `claude/hdash-earnings-overview` | Verified: product, cadence, amount (a reversal reads as a minus with 'Zwrot płatności'), earned date and status as copy. **Added:** 'Do wypłaty od' — the eligible date, a dash when there is none (render-tested). **Missing:** an anonymized customer reference — the workspace RPC returns nothing customer-side by design (D-LINK-04), so a non-identifying ref needs a DB change · **2026-09-17 — date fix:** accounting dates (Data, Do wypłaty od, payout Data) are rendered as Europe/Madrid calendar days with the note 'Daty rozliczeń według kalendarza Madrytu (Europe/Madrid).'; before, the browser zone showed the 1 December deadline as 30.11.2026 west of Madrid (reproduced in `accountingDate.test.ts`, 5 zones, DST, year boundary, leap day, M+3 − 1 s) | Owner decision on that DB change · the rest **READY FOR SERVED OWNER QA** |
...
| J-REF-09 | Referral | Reversed when first purchase fails / void / fully refunded / chargeback / never becomes valid entitlement | ⚪ | ⬜ | ⬜ | ⬜ | 🔓 | — | — · **2026-09-17 (b):** live behaviour differs from this row: ANY succeeded refund, partial included, reverses the whole reward (probe, `reports/GELLATTI_WWU_FINANCIAL_TEST_MATRIX.md` §3). This row says "fully refunded" — owner rule needed | — |
...
Auto ✅ **101** · ⬜ 149. Served ✅ **45** · ⬜ 205 · Owner ✅ 0 · ⬜ 250.
