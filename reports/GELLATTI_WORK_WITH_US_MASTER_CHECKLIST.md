...
| A-IA-07 | DB | `franchise_inquiries.source_route` column + RPC allowlist applied | 🟢 | ✅ | ✅ | ⬜ | 🔓 | migration `20260906200629`, applied 2026-09-06 | Deploy does NOT auto-apply migrations — this was dead until applied by hand · **2026-09-17 — SQL side pinned:** `src/features/franchise/franchiseInquiry.migration.test.ts` — `source_route` is nullable text; the latest RPC definition writes it; its allowlist equals `FRANCHISE_SOURCE_ROUTES` exactly; an unknown route is stored as null, never refused; concepts equal `FRANCHISE_CONCEPT_ORDER` | Owner QA |
...
| C-APP-04 | Application | Duplicate active applications impossible for one account | 🟢 | ✅ | ⬜ | ⬜ | 🔓 | `claude/growth-regression-guards` | `partner_applications_open_uniq` is UNIQUE on `user_id` for exactly the four open statuses (latest definition `20260831201000`); the submit RPC answers a second submit with the existing application (`duplicate: true`) or re-opens the same row after "more information needed". Pinned by `openApplicationUniq.migration.test.ts`, with the open set derived from the canonical status list | — |
...
| C-APP-07 | Application | Status page readable by the applicant | 🟢 | ✅ | ⬜ | ⬜ | 🔓 | `claude/capp07-status-surface` | **Built on `/partner`, not as a new page** — `/partner` is the Affiliate workspace and already owned every application state. Before: the page gate and the panel each hand-wrote status copy and disagreed — `under_review` showed "in progress" above an EMPTY form (a resend only came back as a duplicate); `more_information_needed` said so and offered no way to answer; `suspended`/`terminated` fell through to the form. Now ONE decision (`applicationSurface`) over the canonical `PARTNER_APPLICATION_STATUS_COPY`: each of the 8 DB statuses shows label · meaning · "Twój ruch" (action needed or not) · the available action. More-info pre-fills what was sent (consent never pre-ticked) and shows the team's message; suspended → canonical mailbox; no raw status reaches the page (asserted over visible text, all 8) | **READY FOR SERVED OWNER QA** — one applicant account per state on staging; no QA rows are fabricated in the shared DB |
| C-APP-08 | Notifications | Emails: received · more info requested · approved · rejected · payout setup required · payout/account requirements | 🟡 | ✅ | ⬜ | ⬜ | 🔓 | — | **UNBLOCKED by owner correction §3** — Resend adapter behind a provider-agnostic `EmailProvider` port. Depends on EMAIL-01..06 · **2026-09-17 (b):** review before approval found 3 defects in the unapplied migration, fixed in PR #381 (no DB action): applicant subjects carried the internal bracket taxonomy (ES7) → plain sentence + `[STAGING] `; the app never sent `origin`, so every mail incl. production would say staging → `submitPartnerApplication` sends it; "Tryb/panel Affiliate" → "Partner" (C-APP-13). **Open owner decisions:** admin activation and invitation acceptance also write `approved` (approval mail + always a payout-setup mail, labelled staging); and whether the payout-setup mail should exist, given G-WEL-05/07 say there is nothing to do. Package 2 in `reports/GELLATTI_WWU_DB_APPROVAL_PACKAGES.md`: **NOT READY** until #381 merges and both are decided | Merge #381 (no DB) · owner: decide the two e-mail questions · then approve or decline package 2 |
| C-APP-09 | Approval | On approval: link partner, activate, grant HOME + PRO, grant PARTNER mode, codes available, Connect available, audit row, approval message | 🟢 | ✅ | ⬜ | ⬜ | 🔓 | `claude/growth-regression-guards` | **Verified read-only 2026-09-10** — live = repo `20260831201000` (normalised md5 `f718220b…`): partner row linked and activated · profile APPROVED · entitlements `home` / `pro` / `partner` (source `approved_partner`) · first code minted when none is active · application → approved · in-app `PARTNER_ACTIVATED` → `/partner` · audit `partner.application_approved` · Connect onboarding requires exactly an active partner. Pinned by `approvalGrants.migration.test.ts`. The approval **email** is C-APP-08 (not applied). **Finding:** reject / more-info notifications link to the retired `/work-with-us`, which now redirects to Franchise (2 live rows) — fixed on read in `claude/capp09-notification-links`; the writer fix is migration `20260910220000`, **READY / NOT APPLIED** | **READY FOR SERVED OWNER QA** (an admin approval on staging) |
| C-APP-10 | Approval | **No fake zero-price Stripe subscriptions** for Partner free access | 🟢 | ✅ | ⬜ | ⬜ | 🔓 | `claude/growth-regression-guards` | The approve branch grants access only as `entitlements` rows (source `approved_partner`) and never touches `customer_subscriptions`, `billing_price_catalog` or Stripe — pinned by `approvalGrants.migration.test.ts` | — |
...
| C-APP-13 | Naming | One user-facing name for the signed-in mode. Owner IA says **Affiliate** (hamburger, `/affiliate`); the mode switcher, `/partner` headings and application status copy say **Partner** | 🟢 | ✅ | ⬜ | ⬜ | 🔓 | `claude/capp07-status-surface` | Found during C-APP-07. **OWNER DECISION 2026-09-10: KEEP "Partner"** as the signed-in account/mode name — the authenticated Partner area is not renamed to Affiliate. Public acquisition/campaign language may say "affiliate" where appropriate. C-APP-07 already keeps "Partner", so nothing is renamed · **2026-09-17 — guarded:** `src/copy/workWithUsOwnerRules.guard.test.ts` pins the mode switcher label `Partner` → `/partner`, the dashboard title `Partner`, and no "Affiliate" literal inside /partner (PartnerPage, its affiliate / partner-application imports, `copy/cooperation.ts`). It also keeps unrendered the `copy/affiliate.ts` keys that call the dashboard "Panel Affiliate" (`cta.approved`, `panel.openPanel`, `state.approvedTitle`/`approvedBody`): they are dead copy today, and wiring one in would rename the signed-in area. Served ⬜: needs a signed-in partner | — (decided) · optional owner call: delete or reword the dead "Panel Affiliate" keys |

### D — Partner codes (§8) + campaign links (§9) · CHECKPOINT C

| ID | Area | Requirement | Work | Auto | Served | Owner | Freeze | PR/SHA | Problem / Why | Next Action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| D-CODE-01 | Codes | Normalization + validation: no spaces, no accents, ASCII+digits, 5–16 shown, case-insensitive, protected/offensive rejected | 🟢 | ✅ | ⬜ | ⬜ | 🔓 | `c004d659` | **Re-run as evidence 2026-09-10:** `partnerCodes.test.ts` 23/23 (spaces, accents, ASCII + digits, 4/5/16/17 boundaries, case-insensitive, protected and offensive words) plus the TS ↔ SQL banned-word parity in `partnerCodeSlots.migration.test.ts` — green (133 tests across the four code/attribution suites) | — |
| D-CODE-02 | Codes | **0–3 current public codes per Partner** (X3) | 🟢 | ✅ | ⬜ | ⬜ | 🔓 | `20260831143710` | **DB layer applied and proven against the live staging database** — 0→1→2→3 OK, 4th refused `partner_active_code_limit_reached`, retiring frees a slot, UPDATE contract A–E. The ceiling turned out to be pre-existing (`gellatti_partner_code_guard_v1`); the duplicate this workstream added is dropped. **Live-DB proof is NOT served QA** — no partner has exercised this through the app, because `managePartnerCode` and the UI are not wired · **2026-09-11 — service + UI wired:** the Codes section creates a code (live availability, D-CODE-03 #263) and archives one; the 3-slot limit renders from current codes only, aliases never counted (`partnerCodeSlots.render.test.tsx`, D-CODE-05) | **READY FOR SERVED OWNER QA** — 0→3 codes, 4th refused, archiving frees a slot, through the app |
| D-CODE-03 | Codes | Global uniqueness across every Partner, live availability validation | 🟢 | ✅ | ⬜ | ⬜ | 🔓 | `claude/dcode03-code-availability` | Global unique indexes applied; `gellatti_partner_code_claim_refusal_v1` gives live typed availability (live body = repo `20260831200200`, normalised md5 `069bf0ea…`). **Wired:** the Codes form asks it as the partner types (debounced, only from 5 characters), shows one sentence per refusal and "Kod jest dostępny.", and will not submit a refused code; the copy keys are pinned to the SQL's `return '…'` vocabulary. **Finding (not changed — DB):** the RPC does not check that `p_partner_id` is the caller's own; it can reveal only whether that partner already holds 3 active codes or a given code · **2026-09-17 (b):** the owner-check migration `20260910210000` (PR #281) calls itself "D-CODE-08", but no such row exists — it belongs here. Package 4 in `reports/GELLATTI_WWU_DB_APPROVAL_PACKAGES.md`, **WAITING OWNER DB APPROVAL** | **READY FOR SERVED OWNER QA** (needs a partner account). Owner decided 2026-09-10 to tighten the caller check → prepared as its own migration, **READY / WAITING OWNER DB APPROVAL** |
| D-CODE-04 | Codes | **Historical aliases stay owned by the same partner and can never be claimed by another** (X2) | 🟢 | ✅ | ⬜ | ⬜ | 🔓 | `bd32c0e9`+`b4e3a187` | Fixed in domain (CS2/CS5) and in SQL: uniqueness is now GLOBAL, not active-only. Migration **refuses to run** if duplicate code text already exists, naming the codes — that ownership call moves money · **2026-09-11:** the code UI exists — aliases stay listed as 'Zarchiwizowany' with their history, and a claim of someone else's alias is refused by the availability check (D-CODE-03); history ownership is guarded by `partnerCodeHistory.migration.test.ts` | **READY FOR SERVED OWNER QA** — claim another partner's alias through the app; it must be refused |
| D-CODE-05 | Codes | Aliases don't count toward the 3 displayed slots | 🟢 | ✅ | ⬜ | ⬜ | 🔓 | `20260831143710` + `claude/dash-guards` | DB half proven live (CS3: 3 current + any number of aliases still consume exactly 3 slots; an old alias stays globally reserved). **Displayed half — the row said nothing displays slots; the page already did:** the Overview's "Aktywne kody x / 3" tile and the Codes section's limit count only current codes. Rendered with aliases and a blocked code present: 2 current → "2 / 3" and "Utwórz kod" enabled; 3 current → "3 / 3", the limit message, and the button disabled (the `disabled` attribute, not a class); aliases stay listed. Pinned by `partnerCodeSlots.render.test.tsx` | **READY FOR SERVED OWNER QA** (needs a partner account) |
| D-CODE-06 | Codes | Changing a code never rewrites historical conversions / commission / payout / campaign ownership | 🟢 | ✅ | ⬜ | ⬜ | 🔓 | `claude/growth-regression-guards` | Archiving retires the code row (owner-only, active-only) and writes an audit entry — nothing else; no migration anywhere moves a history row (`referral_clicks`, `referral_attributions`, `commission_entries`, `commission_adjustments`, payouts, `partner_content_links`) to another partner. Pinned by `partnerCodeHistory.migration.test.ts` | — |
| D-CODE-07 | Codes | Admin can disable a compromised alias | 🟢 | ✅ | ⬜ | ⬜ | 🔓 | `claude/growth-regression-guards` | **Already built end to end — the row was stale.** The admin Partners panel lists every code a partner ever held (aliases included: the list filters on the partner only), each with Wyłącz / Włącz; `gellatti_admin_partner_code_action_v1` (live = repo, md5 `64af49ae…`) is Partner-admin only, requires a reason, records blocked + reason + who + when, and writes an audit row. Pinned by `adminCodeDisable.test.ts`. **Finding (DB, not changed):** REACTIVATE always sets `active`, so re-enabling a blocked former alias makes it a current code instead of restoring `retired` | **READY FOR SERVED OWNER QA** (admin) · owner decides whether REACTIVATE should restore the previous status |
...
| D-LINK-02 | Campaign links | All links resolve to immutable `partner_id` | 🟢 | ✅ | ⬜ | ⬜ | 🔓 | `claude/growth-regression-guards` | `partner-link-resolve` reads only slugs from the request and derives the partner from the approved profile; the code must be that partner's active code and a campaign link must belong to that partner and code; the click stores those rows' ids and the claim copies `partner_id` from the stored click (self-referral refused). Pinned by `partnerLinkOwnership.contract.test.ts` | — |
| D-LINK-03 | Campaign links | Aggregate clicks / signups / paid conversions / active subs / per-campaign performance | 🟡 | ✅ | ⬜ | ⬜ | 🔓 | `claude/dlink03-link-performance` | **Audit:** the workspace RPC gave every code clicks, signups, paid customers, revenue and commission, but every campaign link only its clicks — and nothing said how many referred subscriptions are active now. **Built:** migration `20260910200000` (**READY / NOT APPLIED**, rollback in `supabase/rollbacks/`) adds `codes[].activeSubscriptions` and per-link `signups` · `paidCustomers` · `activeSubscriptions` — counts only (D-LINK-04). "Active" reuses the tier rule: `gellatti_partner_active_referred_count_v2` is v1 plus two optional narrowing arguments, contract-tested identical; v1 untouched. The workspace body is otherwise byte-identical to live (md5 `7e674248…`), also contract-tested. **UI now:** code and link statuses read as copy (they were printed raw: `active`, `ACTIVE`/`ARCHIVED`); a link shows exactly the numbers the RPC returns — clicks only until the migration is applied | **WAITING OWNER DB APPROVAL** to apply `20260910200000` (owner 2026-09-10: not yet). The UI half is **READY FOR SERVED OWNER QA** now (statuses as copy, clicks per link); per-link counts after the apply |
...
| D-ATTR-01 | Attribution | 30-day window · explicit code overrides unconverted passive · locked on first paid · one owner per payment · self-referral rejected | 🟢 | ✅ | ⬜ | ⬜ | 🔓 | `claude/growth-regression-guards` | **Re-run as evidence 2026-09-10:** `attribution.test.ts` 32/32 (window, explicit code over cookie, lock on first paid, one owner, self-referral). The DB half is pinned by `attributionOwnership.migration.test.ts`: partial unique indexes allow one ACTIVE owner per subscription, by cache row and by Stripe id | — |
...
| E-GOLD-02 | Tier | Gold automatic from 100 eligible active paid referred subs, HOME+PRO combined | 🟢 | ✅ | ✅ | ⬜ | 🔓 | `a8580c87`+`40120b3f` | Writer built and scheduled monthly. Immutable, idempotent, Madrid boundaries, Elite override wins. Owner scenarios proven: 99 Standard, 100 Gold, drop to 87 leaves February Gold intact and March reads Standard | Served QA once a Partner reaches the threshold on staging |
...
| E-HOLD-02 | Hold | Dashboard shows earned / held / eligible date / eligible / batched / transfer / payout / reversed | 🟡 | ⬜ | ⬜ | ⬜ | 🔓 | — | Workspace RPC returns commission status + payouts; the 8 distinct states are not all surfaced · **2026-09-11:** surfaced — earned date, held ('W trakcie'), eligible date ('Do wypłaty od'), eligible ('Do wypłaty'), paid, reversed ('Zwrot płatności'), render-tested (H-DASH-07). Not surfaced: batched and transfer — the RPC has no such fields (same DB change as F-PAY-03) | Owner: workspace RPC — batch / transfer fields (DB) |
...
| E-REV-04 | Partner copy | The rest of `/partner` reads as copy too — profile moderation (`APPROVED` / `UNDER_REVIEW` / `DISABLED`, Profile section), partner status (`active` / `suspended` / `terminated`) and tier (`standard` / `gold` / `elite`, Settings) | 🟢 | ✅ | ⬜ | ⬜ | 🔓 | `claude/dlink03-link-performance` | Found during D-LINK-03, which fixed the code and link statuses; E-REV-03's guard lists only `String(row.*)` renders, so none of these was caught. Now `partnerAccountDisplay.ts`, its keys checked against each column's CHECK; an unknown value degrades to a dash. UNDER_REVIEW / DISABLED say what the partner needs to know — only an approved profile resolves publicly | **READY FOR SERVED OWNER QA** (needs a partner account) |
...
| F-PAY-01 | Payout | **Monthly payout batch worker** — 1st of month, after reconciliation + tier snapshot | 🟢 | ✅ | ⬜ | ⬜ | 🔓 | `40120b3f` | Full execution layer + pg_cron, scheduled AFTER the tier snapshot. **Live transfers are hard-gated**: `payout_release_state` defaults to not released and every money-moving function asserts it | Payout migration NOT applied — owner-gated (§13 Live OFF) |
| F-PAY-02 | Payout | €25 default minimum; below → carry forward | 🟢 | ⬜ | ⬜ | ⬜ | 🔓 | `c004d659` | P2 | Regression test |
| F-PAY-03 | Payout | Dashboard distinguishes Gellatti batch / Stripe transfer / bank payout | 🟡 | ⬜ | ⬜ | ⬜ | 🔓 | — | Three-table model supports it; UI unverified · **2026-09-11:** the payouts list shows amount, status as copy (Zaplanowana / W realizacji / Wypłacona / Nieudana / Poniżej progu / Do wyrównania / Konto niegotowe), carry-forward and date (H-DASH-08 render test). It cannot tell a Gellatti batch from a Stripe transfer from a bank payout: the RPC returns one status per payout and no batch / transfer / bank fields — a DB change | Owner: workspace RPC — batch / transfer / bank state (DB) |
| F-PAY-04 | Payout | No exact bank arrival date promised | 🟢 | ✅ | ⬜ | ⬜ | 🔓 | `claude/growth-regression-guards` | Verified: partner-facing payout copy says scheduled / in progress / paid and never when money lands. Guarded by `payoutCopyNoDatePromise.test.ts` over the payout and commission status copy, the partner page's Overview / Earnings / Payouts, and every payout line of the public Affiliate and cooperation copy (PL + EN patterns: spans of days, deadlines, guarantees, arrival). H-DASH-02 left the batch-date tile out for the same reason | — |
...
| H-DASH-02 | Dashboard | Top overview: current-month estimated commission · held · eligible · next payout batch | 🟡 | ✅ | ⬜ | ⬜ | 🔓 | `claude/hdash-earnings-overview` | **3 of 4 built, no DB change:** the Overview now leads with 'Prowizja w tym miesiącu' (Madrid month, reversals excluded) · 'W trakcie' (held) · 'Do wypłaty' (eligible), computed from the commission ledger the workspace already returns and labelled with the ledger's own copy (`earningsSummary.ts`, render-tested). **'Next payout batch' deliberately not shown:** the monthly job runs on the 1st (`gellatti-partner-monthly`, `45 2 1 * *`), but production payouts stay disabled until an owner release (F-PAY-06) and no bank date may be promised (F-PAY-04), so a batch date would read as a payout promise | **WAITING OWNER DECISION** — whether, and in what words, to show the batch date while production payouts are disabled · the rest **READY FOR SERVED OWNER QA** |
...
| H-DASH-06 | Dashboard | Referral tools: 3 codes · copy · public URLs · campaign builder · copy links · per-campaign performance | 🟡 | ✅ | ⬜ | ⬜ | 🔓 | `claude/hdash06-copy-buttons` | `Codes` + `LinkGenerator` + `ContentLinks` exist · **2026-09-11 — copy in one click:** each current code shows its public link (`/<profil>/<kod>`) with 'Kopiuj kod' / 'Kopiuj link'; the generator's ready link and every campaign link get 'Kopiuj link' (`CopyValueButton`: 'Skopiowano' for a moment, a refused clipboard changes nothing — `copyValueButton.test.tsx`, `partnerCopyButtons.render.test.tsx`). 3 slots: D-CODE-05. Campaign builder: `LinkGenerator`. Per-campaign performance: rendered (D-LINK-03, #262); its sign-up, paying-customer and active-subscription counters (Rejestracje · Klienci · Aktywne subskrypcje; corrected 2026-09-17, the migration adds no unique-visitor count) wait for migration 20260910200000 (NOT APPLIED) | **READY FOR SERVED OWNER QA** (copy, public links) · Owner: apply 20260910200000 for the per-campaign counters |
| H-DASH-07 | Dashboard | Commissions list: product · cadence · amount · earned · eligible · status · reversal · anonymized customer ref | 🟡 | ✅ | ⬜ | ⬜ | 🔓 | `claude/hdash-earnings-overview` | Verified: product, cadence, amount (a reversal reads as a minus with 'Zwrot płatności'), earned date and status as copy. **Added:** 'Do wypłaty od' — the eligible date, a dash when there is none (render-tested). **Missing:** an anonymized customer reference — the workspace RPC returns nothing customer-side by design (D-LINK-04), so a non-identifying ref needs a DB change | Owner decision on that DB change · the rest **READY FOR SERVED OWNER QA** |
| H-DASH-08 | Dashboard | Payouts: statement · amount · batch · transfer · bank state · failure messages · negative carry-forward | 🟡 | ✅ | ⬜ | ⬜ | 🔓 | `claude/hdash-earnings-overview` | Verified and render-tested: amount; status as copy — a failure reads 'Nieudana — Przelew się nie powiódł… kwota nie przepada' and the provider's raw failure code is never printed; carry-forward ('Przeniesienie', negative allowed); date. **Not available:** statement, batch, Stripe transfer and bank state — the workspace RPC returns none of them (DB change) | DB change for batch / transfer / bank state → owner · the rest **READY FOR SERVED OWNER QA** |
| H-DASH-09 | Dashboard | Settings: profile · Connect onboarding/update · notification prefs · terms/status | 🟡 | ⬜ | ⬜ | ⬜ | 🔓 | — | Verified by reading the page: public-profile editing (Profile) · Connect onboarding ('Dokończ konfigurację' when an account exists but payouts are off — Payouts section) · status and tier (Settings; as copy after E-REV-04). **Missing:** notification preferences (none on /partner) and a terms link | Product decision: which notification preferences, and which terms document to link |
| H-DASH-10 | Dashboard | Cancelled subscriptions visible as business effect without PII: Active · Cancels at period end · Ended · Payment failed · Refunded/reversed | 🟡 | ⬜ | ⬜ | ⬜ | 🔓 | — | Not surfaced today · **2026-09-11:** of the five, only refunded / reversed is visible — a reversal reads 'Zwrot płatności' with a minus (H-DASH-07 render test). Active · cancels at period end · ended · payment failed need a per-attribution subscription state the workspace RPC does not return (it has aggregate counts only) — a DB change, no PII | Owner: workspace RPC — per-attribution subscription state without PII (DB) |
| H-DASH-11 | Dashboard | Future renewals stop creating commission after paid entitlement ends | 🟢 | ✅ | ⬜ | ⬜ | 🔓 | `c004d659` + `claude/dash-guards` | Commission is booked only from a PAID invoice: exactly `invoice.paid` / `invoice.payment_succeeded` route to the writer, which is reached from one place; a subscription's end and a failed renewal book nothing; an unpaid or zero-value invoice is refused by the writer (C6). Pinned by `renewalCommissionStops.test.ts` · **2026-09-17 (b):** the T-TEST-02 inventory found the tests pin routing, eligibility and the single call site, but never run the writer; and the writer never reads subscription status — a paid invoice on a cancelled subscription still books. The rule holds only while Stripe issues no paid invoice after the end. No positive control shows a second paid renewal booking a second entry. Status unchanged; see `reports/GELLATTI_WWU_FINANCIAL_TEST_MATRIX.md` §2.8 · **2026-09-17 (c):** positive control added: `stripeWebhookDispatch.test.ts` — a paid renewal on the locked attribution books a second entry (breaking the active-attribution lookup fails it). The stop condition itself still depends on Stripe issuing no paid invoice after the end (the writer does not read subscription status) | — |
...
| I-ADM-02 | Admin | Partner list with search/filter, status, tier, active HOME/PRO/total, codes, Connect status, held, eligible, lifetime paid | 🟡 | ✅ | ⬜ | ⬜ | 🔓 | `claude/iadm02-partner-list-search` | **Search and filter built, no DB change:** find a partner by e-mail, name, slug, any code (aliases too) or id, narrow by status, see 'Wyniki: X z Y'; each row now shows the tier. Already shown: status, codes (with disable), Connect, payouts, clicks, attributions, pending (held + eligible). **Missing — the admin list RPC does not return them:** active HOME / PRO / total, held vs eligible split, lifetime paid (DB change) | Owner: admin-list RPC fields · the rest **READY FOR SERVED OWNER QA** (admin) |
...
| I-ADM-05 | Admin | Sensitive actions require confirmation + reason + audit | 🟡 | ✅ | ⬜ | ⬜ | 🔓 | `claude/iadm02-partner-list-search` | **Partners panel, no DB change:** every sensitive action — suspend, reactivate, code / link disable and re-enable, profile approve / disable, logo removal, Connect provisioning, a new commission version, activating an existing user — now opens a confirmation that says what will happen; nothing fires on one click. The reason is no longer prefilled ('Admin Partner operation' / 'Admin activation' satisfied the server check and filled the audit log with the same words): it is asked in the confirmation and required wherever the RPC records one (all but Connect). Escape and the backdrop cancel. **Audit, checked in the RPC definitions:** every one of these calls `gellatti_write_audit_v1`, and status / profile / link / code / commission refuse a blank reason (`adminPartnerActionConfirm.test.ts`, `adminPartnersSection.confirm.test.tsx`). **Open:** the activation RPC records the reason but does not refuse a blank one (DB change); the applications queue's approve / reject / more-info still take an optional reason and no confirmation (I-ADM-01) | Owner: activation RPC reason check (DB) · applications-queue reason policy · the rest **READY FOR SERVED OWNER QA** (admin) |
| I-ADM-06 | Admin | **"Preview as Partner" is strictly read-only** — no impersonated writes | ⚪ | ⬜ | ⬜ | ⬜ | 🔓 | — | Security-critical · **2026-09-11:** nothing to build without the DB — `gellatti_partner_workspace_v1` answers only for `auth.uid()`, so a read-only preview needs a new admin-scoped read RPC; the page would then render the Partner sections from it with every write control absent, plus the write-refusal test | Owner: approve an admin read-only workspace RPC (DB) |

### J — Normal-user referral program (§18–§21) · CHECKPOINT F

**Crosswalk to Lane B, 2026-09-17 — no J status changed.** Lane B (K01–K22) built and live-proved a refer-a-friend programme on owner rules F1–F9 (`src/features/referral/referralRewardRules.ts`, 2026-09-02). These J rows come from the owner brief §18–§21 as audited on 2026-08-31, and some of them contradict what is live:

- **J-REF-04:** annual first purchase gives +3 months here, but +30 days live (K05). J-REF-05's "4 annual = 12 months" inherits the same conflict.
- **J-REF-06:** here a HOME referrer's HOME is extended and HOME is never silently upgraded to PRO; live, a HOME referrer gets temporary PRO days (K06).
- **J-REF-08 / J-REF-16:** four states here (ZDOBYTE / OCZEKUJE / WYKORZYSTANE / COFNIĘTE); live has two, earned and reversed (K16).
- **J-REF-12:** "really changes entitlement/billing time" here; live changes the entitlement but never the billing cycle (F4, K09).

Agree with what is live: J-REF-01 (K03), J-REF-02 (K12), J-REF-03 (K04), J-REF-09 (K13, K14), J-REF-11 (F9), J-REF-14 (K01). Not built or not proven: J-REF-07 (store credit — live uses the PRO Bonus Bank, K08), J-REF-10 (no explicit rule found), J-REF-13 (Stripe test-clock proof; K19–K22 blocked), J-REF-15 (WhatsApp / Facebook share). **Owner decision needed:** which spec is canonical, F1–F9 (live) or §18–§21 (these rows). J statuses stay as they are until then.
...
| J-REF-02 | Referral | FIRST PAID PURCHASE ONLY; later renewals create no further reward | ⚪ | ⬜ | ⬜ | ⬜ | 🔓 | — | — · **2026-09-17 (b):** the SQL's one-live-reward index is `where status = 'earned'`, so after a reversal a later renewal invoice can earn again (the SQL calls it a "genuine later qualification"). This row says renewals earn nothing — owner rule needed; forbidding it is a SQL migration | — |
...
| J-REF-09 | Referral | Reversed when first purchase fails / void / fully refunded / chargeback / never becomes valid entitlement | ⚪ | ⬜ | ⬜ | ⬜ | 🔓 | — | — · **2026-09-17 (b):** live behaviour differs from this row: ANY succeeded refund, partial included, reverses the whole reward (probe, `reports/GELLATTI_WWU_FINANCIAL_TEST_MATRIX.md` §3). This row says "fully refunded" — owner rule needed · **2026-09-17 (c):** a full refund reversing the reward is now pinned (`referralWebhookWiring.test.ts`), with a pending refund reversing nothing; the partial-refund rule is left unpinned until the owner decides | — |
...
| J-REF-17 | Referral UX | Never expose referred person's PII | 🟡 | ✅ | ⬜ | ⬜ | 🔓 | `src/features/referral/referralDashboardPrivacy.migration.test.ts` | — · **2026-09-17:** the three user RPCs are pinned: a reward row is exactly id · product · cadence · bonusDays · status · earnedAt; a claim answers only ok + reason; minting returns only the caller's own code; nothing identifying the other person is read. **FINDING (read-only, live DB 2026-09-17):** `authenticated` holds table-level SELECT on `referral_rewards` and `user_referral_attributions`, and the own-row policies let a referrer read `referred_user_id`, `attribution_id`, `stripe_subscription_id`, `stripe_invoice_id` and `reversal_reason` on their own reward rows through the REST API. Latent: both tables hold 0 rows today, and no client reads them directly. Closing it is a grant change: package 6 in `reports/GELLATTI_WWU_DB_APPROVAL_PACKAGES.md` (migration `20260917120000_referral_tables_revoke_direct_read`, PR #380, open by design), **WAITING OWNER DB APPROVAL** | Owner: approve or decline package 6 (the referral table grants), separately from the five pending migrations |
...
| L-PRICE-02 | Prices | Public wording: transport and destination tax/VAT handled in the final quote; no invented delivered pricing | 🟢 | ✅ | ✅ | ⬜ | 🔓 | `src/copy/workWithUsOwnerRules.guard.test.ts` | Quotation is **EXW China** (Hangzhou Gelato Tech Co., Ltd) — see N-TRAIL-05 for the related Incoterm risk · **2026-09-17:** the copy already follows it — `/machines` "Transport, podatki i montaż zależą od kraju i lokalu. Podajemy je w wycenie…", `/trailer` "…transport i podatki dobieramy do projektu". Guard: no Incoterm in any public literal, no delivered-pricing phrase on the lanes, and the only price on the lanes is the owner's trailer sentence. **Served 2026-09-17** (staging.pinguinoai.com, origin/staging `6e83b8d1`): as described; the only figure on the four lane pages is €10,000 on /trailer | Owner QA |
...
| L-STORY-02 | Story | **Manufacturer name is INTERNAL ONLY.** Public pages name the model (V2, V4B, Battery Cart, Milano) with no manufacturer attribution, and never imply Gellatti manufactures the equipment | 🟡 | ✅ | ✅ | ⬜ | 🔓 | `src/copy/workWithUsOwnerRules.guard.test.ts` | **REVERSED by owner correction §5/§14.** Supplier identity stays in procurement, evidence and Admin only. Needs an automated guard so the name cannot leak into a public bundle · **2026-09-17 — guard landed:** fails if any public literal (admin excluded) names the manufacturer, brand, series or domain recorded in `reports/GELLATTI_MACHINE_SPEC_RECONCILIATION.md`, or claims that Gellatti manufactures machines, carts or trailers; today none does. **Served 2026-09-17** (staging.pinguinoai.com, origin/staging `6e83b8d1`): /trailer, /machines, /mobile, /franchise carry no manufacturer identity. Still 🟡: no public page names a model yet (catalogue not built), so "public pages name the model" is unproven | Name models when the machine catalogue lands (the guard already holds) · Q-ASSET-12 covers images |
...
| N-TRAIL-05 | Trailer | **Never publish "FOB Germany".** Public wording is exactly: "Przyczepa bazowa od €10,000. Lokalizacja: Niemcy. Maszyna, wyposażenie, branding, transport i podatki dobieramy do projektu." | 🟢 | ✅ | ✅ | ⬜ | 🔓 | `src/copy/workWithUsOwnerRules.guard.test.ts` | **RESOLVED by owner correction §9** — the incorrect Incoterm is banned outright rather than deferred · **2026-09-17:** the owner's exact sentence is `TRAILER_PAGE.next` (`src/copy/workWithUsLanes.ts`) and renders on `/trailer`; "FOB" survives only in comments that name the ban. Guard: no public literal (admin excluded) contains FOB or any Incoterm, and the sentence stays exact. **Served 2026-09-17** (staging.pinguinoai.com, origin/staging `6e83b8d1`): `/trailer` shows the exact sentence; no FOB or Incoterm on /trailer, /machines, /mobile, /franchise | Owner QA |
...
| P-LEAD-06 | Leads | Admin notified at `info@gellatti.com` via the canonical notification system | 🟢 | ✅ | ✅ | ⬜ | 🔓 | migration `20260910032351`, applied+verified 2026-09-10 | — | **UNBLOCKED by owner correction §1–§3.** Subjects must use the §2 taxonomy | After EMAIL-01..06 · **DONE.** Queued through the canonical `gellatti_enqueue_email_v1` (owns idempotency), subject_key `franchiseInquiryNew` from the existing closed taxonomy, area FRANCHISE, recipient info@gellatti.com. Verified live both ways: staging origin → `[STAGING]` label + environment staging; gellatti.com origin → production, no label. Environment is CLASSIFIED server-side from the caller origin, not claimed, because prod and staging share one DB. Enqueue failure cannot roll back the lead. QA rows removed. · **2026-09-17 — now pinned by a test:** `src/features/franchise/franchiseInquiry.migration.test.ts` reads the LATEST RPC definition: one enqueue through `gellatti_enqueue_email_v1`, recipient info@gellatti.com, subject key `franchiseInquiryNew` equal to the taxonomy prefix (incl. `[STAGING]`), idempotency key per enquiry, origin → environment defaulting to staging, the lead survives an enqueue failure, anon execute never revoked. The client-side origin is pinned by `src/services/franchiseOrigin.test.ts` |
...
| ID | Area | Requirement | Work | Auto | Served | Owner | Freeze | PR/SHA | Problem / Why | Next Action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| T-TEST-01 | Tests | Every checklist row references its tests | 🟡 | ⬜ | ⬜ | ⬜ | 🔓 | — | — · **2026-09-17 — the 64 rows this workstream changed are mapped to their tests:** `reports/GELLATTI_WWU_TEST_COVERAGE_MAP.md` — full 38 · partial 17 · none 5 · n/a 4; all 46 mapped test files pass (594 tests). The remaining rows are not mapped yet · **2026-09-17 — gaps closed the same day:** P-LEAD-06 and A-IA-07 (`franchiseInquiry.migration.test.ts`), C-APP-13 (`workWithUsOwnerRules.guard.test.ts`), Lane B K16 (`referralRewardRow.render.test.tsx`); new guards also cover N-TRAIL-05, L-STORY-02, L-PRICE-02, O-FRAN-02 and J-REF-17 | Map the remaining rows · still open: EMAIL-01 (to build), G-WEL-04 and I-ADM-06 (need DB), J-REF-17 table grants (package 6) |
| T-TEST-02 | Tests | Financial matrix: duplicate/out-of-order webhooks, fail, success, refunds, partial, disputes, renewals, annual, monthly, monthly→annual, idempotency, concurrency | 🟡 | ✅ | ⬜ | ⬜ | 🔓 | — | Large parts exist in the domain tests + `WEBHOOK_MATRIX.md` · **2026-09-17 (b):** inventory done: `reports/GELLATTI_WWU_FINANCIAL_TEST_MATRIX.md` — 13 dimensions × commission and reward lanes, 20 files / 694 tests. **Commission:** duplicate, failed, success, full + partial refund, annual and idempotency COVERED; out-of-order, dispute, renewals, monthly, monthly→annual, concurrency PARTIAL. **Reward lane:** out-of-order, full + partial refund, renewals, monthly→annual, concurrency GAP. **Findings (not changed):** (1) a refund, void or dispute processed BEFORE `invoice.paid` leaves the commission and the reward unreversed — needs a `dispatch.ts` change, no DB; (2) the writer never reads subscription status, so a paid invoice on a cancelled subscription still books (H-DASH-11 relies on Stripe); (3) a won dispute is not re-credited — needs a DB change (0018 kind vocabulary); (4) five `unique`-index assertions in `billingPlatform.migration.test.ts` would pass without `unique` · **2026-09-17 (c):** every test-only gap from the matrix is closed, each test mutation-checked: `billingPlatform.migration.test.ts` — the six partial-unique index assertions now require `create unique index`; `stripeWebhookDispatch.test.ts` — `invoice.paid` ∥ `invoice.payment_succeeded` and `charge.refunded` ∥ `refund.created` processed concurrently give ONE entry / ONE reversal (first test to run the 23505 backstop: breaking it had failed nothing before), a paid renewal books a second entry on the locked attribution, monthly then annual books 199 then 900; `stripeWebhookEventOrder.test.ts` — ORDER C (invoice first) converges for all four offers; `referralWebhookWiring.test.ts` — a full refund reverses the reward, a pending refund does not, a replay stays quiet, a failing recorder is retryable. **Still open, not test-only:** a refund / void / dispute processed before `invoice.paid` is never reversed (a `dispatch.ts` fix, no DB); a won dispute is not re-credited (DB); a paid invoice after cancellation still books, a partial refund reverses the whole reward, a renewal after a reversal can earn again (owner rules); real Postgres locking (needs a DB test harness) | Owner: rules for the open findings · then a proposal for the refund-before-booking fix in `dispatch.ts` (no DB) |
| T-TEST-03 | Tests | Code matrix: collision, edit/history, 3 slots, campaign attribution, 30-day, self-referral | 🟢 | ✅ | ⬜ | ⬜ | 🔓 | — | Partly covered · **2026-09-11 — every cell mapped to a test:** collision — `partnerCodes.test.ts` PC1–PC4 + `codeAvailability.test.ts` · edit/history — `partnerCodeHistory.migration.test.ts` · 3 slots — `partnerCodeSlots.test.ts` / `.migration` / `.render` · campaign attribution — `partnerLinkOwnership.contract.test.ts` + `attributionOwnership.migration.test.ts` · 30-day — `attribution.test.ts` A1 (incl. the exact-30-days boundary) · self-referral — `attribution.test.ts` A6/A7 | Keep each new code rule in one of these files |
...
| T-SQA-01 | Served QA | Every checkpoint served-verified on staging: signed out, signed in, role, desktop, mobile, empty/loading/error/success, permission boundaries, real DB, real Stripe Sandbox | ⚪ | ⬜ | ⬜ | ⬜ | 🔓 | — | localhost alone is never accepted · **2026-09-17:** a served owner QA script for the 22 READY rows and Lane B K16 — `reports/GELLATTI_WWU_SERVED_OWNER_QA_SCRIPT.md` (route, account, clicks, expected, PASS/FAIL and whether it mutates data, per step). Not run: served QA stays with the owner | Per checkpoint |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| REG-01 | Partner application | **`gellatti_submit_partner_application_v1` fails on every call** | 🟢 | ✅ | ✅ | ⬜ | 🔓 | broke `20260831154203` · fixed `20260831155647` | **Caused by this workstream and now corrected forward.** `20260831201000` changed the audit `actor_type` from `'user'` to `'customer'`; `audit_log_actor_type_check` allows only `system \| admin \| user \| webhook`, so both branches failed. `20260831201100` restores `'user'` — proven pre-apply to be the ONLY semantic change (61 executable lines compared line-by-line against the live body, **0 mismatches**). Live after fix: new submission `submitted` with `audit_actor=user`, resubmit `resubmitted=true` with `audit_actor=user`. **`20260831201000` must never be described as fully successful without naming this correction** | OWNER QA |
| REG-02 | Partner application | Full application lifecycle A–E works end to end | 🟢 | ✅ | ✅ | ⬜ | 🔓 | `20260831155647` | Proven live, all rolled back: **A** submit→`submitted`, audit `user` · **B** `request_information`→`more_information_needed`, audit `admin` (it could **never** succeed before — it wrote the illegal `in_review`) · **C** resubmit→`submitted`/`resubmitted=true`, audit `user`, **one row** not a duplicate · **D** approve→`approved`, partner created, audit `admin` · **E** reject→`rejected`, audit `admin`. `illegal_actor_rows=0` | OWNER QA |
| REG-03 | Partner approval | Partner-code collision probe is case-insensitive | 🟢 | ✅ | ✅ | ⬜ | 🔓 | `20260831154203` | A second latent break this workstream caused: `20260831200000` made `partner_codes_code_global_uniq` case-insensitive (`upper(code)`) while the approve path still probed `where code = v_code`. **Proven live with a deliberate collision**: seeded lowercase `qatestcode`, approval returned **`QATESTCO1`** — the collision was detected and a suffix appended. A case-sensitive probe would have missed it and failed on the unique index | OWNER QA |
...
**Ledger correction 2026-09-10 — duplicate IDs.** The three email rows in this table were numbered `EMAIL-01..03`, the same IDs as the first three rows of the Email lane (§EM) — two different requirements under each ID. They are renumbered `EMAIL-DB-01..03`; their content and history are unchanged. Every other mention of `EMAIL-01..08` in this file refers to the Email lane.
...
| REG-04 | Partner application | Authorization negative controls | 🟢 | ✅ | ✅ | ⬜ | 🔓 | `20260831155647` | Proven live: anon submit **REFUSED 42501** (privilege layer, not just the body guard) · non-admin decision action **REFUSED** `partner_administrator_required` · applicant self-approve **REFUSED** · applicant self-reject **REFUSED** · cross-user application UPDATE **BLOCKED 0 rows** and read **0 rows** (RLS) · gate is still the canonical `gellatti_admin_has_permission_v1('PARTNER')`, true for admin / false for non-admin | OWNER QA |
| REG-05 | Partner application | Audit actor-type contract, owner-locked | 🟢 | ✅ | ✅ | ⬜ | 🔓 | `auditActorTypes.test.ts` | Parses the legal set from the migration that declares the CHECK rather than retyping it, and resolves each function's **latest** definition rather than grepping history. **Both mechanisms proven to catch drift**: injecting `'operator'` into the current definition turns it red; narrowing the canonical constraint turns it red. Applied history is asserted to still carry the superseded `'customer'`, so the resolution is proven to be doing real work | Keep green |
...
| EMAIL-DB-01 | Email | **Persisted email jobs — DB layer** | 🟢 | ✅ | ✅ | ⬜ | 🔓 | `20260831175103` | Applied verbatim after an uncontended full sweep at EXIT 0. Live: enqueue→`queued`; replay→`deduplicated=true`, same id, 1 row; claim→`sending`/attempts 1; re-claim→0; blank provider id **REFUSED**; sent carries provider id + `sent_at`; claim after sent→0; direct false-sent **REFUSED** `check_violation`; retryable→`failed` with no evidence and a scheduled retry; permanent→`abandoned`; bad/missing `area` **REFUSED**. **0 real provider calls** | Adapter/runtime step |
| EMAIL-DB-02 | Email | Admin email visibility is **super_admin only** | 🟢 | ✅ | ✅ | ⬜ | 🔓 | `20260831175103` | Gate moved off `PARTNER` before apply. Proven live with a **seeded `partner_admin`** holding `PARTNER=true`: **REFUSED** `administrator_required`. anon and ordinary authenticated **DENIED 42501** on every path; `super_admin` reads. Bodies excluded from the list surface | OWNER QA |
| EMAIL-DB-03 | Email | Domain vocabulary in three-way parity | 🟢 | ✅ | ✅ | ⬜ | 🔓 | `emailJob.migration.test.ts` | Declared `EmailArea` union, `OPERATIONAL_SUBJECTS` usage and the SQL CHECK compared independently. **All four drift directions proven red** by injection. `SHOP`/`LEAD` absent by design — adding one must change all three together | Keep green |
| GOLD-01 | Tier | **Monthly tier snapshot writer** — Madrid boundary, 100 threshold, Elite precedence, immutable | 🟢 | ✅ | ✅ | ⬜ | 🔓 | `20260831190352` | Applied and proven live: 99 standard / 100 gold / 101 gold; Feb gold/100 → Mar standard/87 with Feb unchanged; re-run wrote 0 | OWNER QA |
| GOLD-02 | Tier | **Historical reconstruction from its own boundary**, never today's count | 🟢 | ✅ | ✅ | ⬜ | 🔓 | `20260831190352` | Case A: Feb reconstructs **gold/105** while today is 87. Case B: Feb reconstructs **standard/87** while today is 105. Both contradict the live count and match their own boundary | OWNER QA |
| GOLD-03 | Tier | **Unknown at the threshold refuses rather than guessing** | 🟢 | ✅ | ✅ | ⬜ | 🔓 | `20260831190352` | 99 provable + 1 unprovable → blocker `missing_initial_state`, **0 snapshots**, typed gap. Decided neither 99/Standard nor 100/Gold | OWNER QA |
| GOLD-04 | Tier | **A live-state writer can never manufacture historical truth** | 🟢 | ✅ | ✅ | ⬜ | 🔓 | `20260831194031` | Past month **REFUSED** `historical_month_requires_catchup` with zero mutation; future month and future measurement instant also refused; catch-up remains the only route to a past month and still returns gold/105 for it. Grants unchanged | OWNER QA |
| GOLD-05 | Tier | Same-month late execution window | ⚪ | ⬜ | ⬜ | ⬜ | 🔓 | — | **OWNER_DECISION_REQUIRED.** No execution-window authority exists and the intended cadence already drifts hours (`45 2 1 * *`). Structure carries the safety today — catch-up runs before the writer and covers the current month — so the residual is a manual mid-month `service_role` call. Closing it needs an owner-set drift constant, deliberately not invented | Owner sets the constant, or accepts the structural mitigation |
| DB-EXT-01 | DB | **Staging is written by more than this lane** | ⚪ | ⬜ | ⬜ | ⬜ | 🔓 | `20260831162207` | `shop_orders_expected_checkout_total` was applied by another session at 16:22, mid-sequence. Zero intersection with partner/email/rate work. Recorded because pre-apply audits must re-read the live register rather than trust a previous snapshot | Owner awareness |
| DB-DRIFT-01 | DB | **Repo migration versions match the versions recorded in the shared DB** — otherwise `supabase db push` re-runs applied files | 🟡 | ⬜ | ⬜ | ⬜ | 🔓 | read-only audit 2026-09-10 vs origin/staging `ca3343d4` | **67** repo versions are not recorded; **94** recorded versions have no same-version file; two repo version collisions (`20260824140000`, `20260824150000`). Cause: `apply_migration` records its own timestamp. A `db push` would re-run the 67, many of them `create or replace` of functions that later migrations changed — it could silently revert live functions. #254 and #260 reconcile 3 (pending merge) · **2026-09-17 — A/B reconciliation plan, analysis only (nothing renamed, no DB action):** `reports/DB_DRIFT_01_AB_RECONCILIATION_PLAN.md`. On `6e83b8d1` (A 11 · B 31 · C 16 · D 13): 36 dependency groups — 32 movable, 3 blocked by class-C files (G14, G15, G33), 1 owned by PR #198 (G29). This workstream: 7 movable groups, in order G21 → G26 → G18 → G19 → G35 → G34 → G36, each listing the tests, sources and filename pins to update in the same PR; G33 is blocked by C `partner_code_slots_and_alias_ownership` | Owner: approve the per-group PR sequence and a replay proof on a disposable database (a Supabase branch is a project resource) · decide what the class-C files record (unblocks G14, G15, G33) · still no `db push` |
...
Auto ✅ **102** · ⬜ 148. Served ✅ **45** · ⬜ 205 · Owner ✅ 0 · ⬜ 250.

**Row-based** — recomputed from the rows themselves at `origin/staging` `b668349b` on 2026-09-17. The block used to be maintained by hand and had drifted (it said 203 rows; see *Summary-block drift* below). Recount it from the rows, never from this block.
