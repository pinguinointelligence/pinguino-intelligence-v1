...
| C-APP-13 | Naming | One user-facing name for the signed-in mode. Owner IA says **Affiliate** (hamburger, `/affiliate`); the mode switcher, `/partner` headings and application status copy say **Partner** | ⚪ | ⬜ | ⬜ | ⬜ | 🔓 | — | Found during C-APP-07. Renaming inside one surface would MIX the names on one screen (switcher "Partner", card "Affiliate"), so C-APP-07 deliberately kept "Partner". A rename has to cover `AccountModeSwitcher`, `/partner` headings, `cooperation.ts` state copy and `PARTNER_APPLICATION_STATUS_COPY` together | **WAITING OWNER DECISION** — rename the mode to Affiliate everywhere, or keep "Partner" as the signed-in mode name |
