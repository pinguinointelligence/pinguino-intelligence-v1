...
| D-CODE-02 | Codes | **0–3 current public codes per Partner** (X3) | ✅ | ✅ | ✅ | ⬜ | 🔓 | `20260831143710` | **Applied + proven live on staging.** The ceiling was already enforced by the pre-existing `gellatti_partner_code_guard_v1`; the duplicate trigger this workstream added is dropped. Live: 0→1→2→3 OK, 4th refused `partner_active_code_limit_reached`, retire frees a slot. UPDATE contract A–E proven. **Service/UI still not wired** | Wire `managePartnerCode`; then OWNER QA |
...
| D-CODE-05 | Codes | Aliases don't count toward the 3 displayed slots | ✅ | ✅ | ✅ | ⬜ | 🔓 | `20260831143710` | CS3 proven live: 3 current + aliases still consumes exactly 3 slots; the old alias stays globally reserved (`held_by_another_partner`) | OWNER QA after the UI is wired |
