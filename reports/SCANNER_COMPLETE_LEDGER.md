...
| SOL-047 | DESIGN — parent item for the Scanner's visual work. Design is the second-to-last phase; in this workstream only the flicker and the technical sentences are removed, nothing is restyled. | TODO | — |
...
| SOL-048 | The Scanner shows micro-second, changing messages and lets late responses overwrite the current state. A functional defect, not only DESIGN. | TODO | owner QA 2026-09-06: "komunikaty zmieniają się tak szybko, że klient nie jest w stanie ich przeczytać" |
| SOL-049 | A new product recognised by exact EAN does not complete the full enrichment and Mapper Rescue: it ends as a private product or as an automatically "reported for verification" one. To be solved systemically over the product matrix, never as a Nestea/Haribo exception. | TODO | owner QA 2026-09-06; DB trace: Nestea CA-ING-007186 saved ready but presented as private-only; Haribo showed a "reported" screen replayed from an open 2026-09-04 request, with no new record created |
