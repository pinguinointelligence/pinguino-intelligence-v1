...
## Internal closeout additions (2026-09-05)
- **Offline persistence** (`offline/persistentStore.ts`): one `KeyValueStore` contract, three backends (memory, Web Storage, IndexedDB without a new dependency). Cache entries carry schema, resolution time, version pointer and account; trusted only while schema matches, TTL holds and the version pointer is not contradicted; online resolutions always overwrite. Namespaces per account and `guest`. The cache is a convenience, never an authority.
- **Provenance** (`discovery/contracts.ts` `Fact`): value · source · authority (scan-core, product-scan-analyze, intimport-enrich, resolve_exact_products_by_gtin_v1) · recordedAt · confidence · contributingSources; ledger context gtin/symbology/session/time. Provider-vs-provider disagreements are resolved server-side by the legacy source rank and V2 shows every contributor; the losing value is not carried by the legacy payload (limitation recorded).
- **QA harness** (`src/pages/dev/ScanImportV2LabPage.tsx`, route `/dev/scan-import-v2`): gated by dev or `VITE_SCAN_IMPORT_LAB=1`; consumes only the V2 boundary; outcomes in plain language (known product, unknown in discovery, ambiguous, offline known/unknown, invalid); label photo, family confirmation and request steps drive the same authorities. Not HOME.
