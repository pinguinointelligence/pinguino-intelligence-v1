...
- Branch previews of the staging project had **no Supabase configuration** (`VITE_SUPABASE_URL` / `VITE_SUPABASE_ANON_KEY` exist for the Production target only), so the harness reported "Supabase nie jest skonfigurowane" on the preview. Fixed without owner action: the two **public** values (URL + legacy anon key, identical to the ones embedded in the served staging bundle) were added to the Preview target **scoped to the `claude/scan-import-v2` branch only** (`vercel env add … preview claude/scan-import-v2`). No other branch preview gained Supabase access; removal is one `vercel env rm` per variable.
