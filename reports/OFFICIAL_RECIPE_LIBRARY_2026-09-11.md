...
`claude/official-recipe-library`, draft PR #286. Processing Rules (Workstream 3) are out of scope.

## 1. Baseline and gate

- Branch base: staging `c65e52eff72a7829ba3209546d5a162e2cfd8fa5`. Staging has since moved to
  `0b42b206` (PR #284, scanner — 2 files, no overlap with this branch).
- **Owner §20 gate: NOT MERGED.** The final Mapper/Search runtime (SA-11/SA-12 + 2541 projection) is
  not on staging yet (the Codex integration worktree holds uncommitted work, no PR). This branch waits
  and is reconciled once it lands. A read-only watcher polls staging.
- Production (`main`) untouched. No shared-database write was made.
...
| `mapper_basement.csv` FINAL 2541 × 62 | `a6a849a596acef75e0760992353bddf5cbca24ff37744e36414da18ca45556f6` | PI identity authority (audit only, never written) |
...
- **Gellatti tab:** five collection cards (owner heroes) → recipe grid (`GEL-NNN-480.webp`) → recipe
  ...
  „Instrukcja procesu w przygotowaniu"), verbatim process notice. Owner-review tooling (Fantasy,
  curated research, countries) stays for admins only.
- **Use (PRO):** `/pro/recipe?source=official_recipe&officialRecipe=<id>` → `openOfficialRecipe`:
  detached copy → Mapper row per PI → existing `resolveCountryProductsForSlots` → exact PR (own
  profile → own composition; otherwise Mapper science + exact identity; no route → canonical PI) →
  ProductBehavior → Engine mass check → new unsaved draft. BRAK, scaffold Main and PIs the runtime does
  not serve block before the store changes. One-shot URL consumed after success.
...
1. FINAL 2541 marks **PI-ING-000618** (rose paste, #72) and **PI-ING-001705** (Leagel vanilla paste,
   #15 #19 #34 #39 #123 #138 #150 #153 + four BRAK recipes) not approved for base/engines. The live
   runtime still approves all 113 PIs, so every BRAK-free recipe opens today; once FINAL is the runtime
   those 9 recipes block honestly at „Użyj receptury".
2. The live runtime (2147 rows) and the repository projection (2089) still carry the **old names** for the
   16 drift PIs. The library shows only the Mapper runtime's name by PI, so FINAL names appear when the
   Mapper/Search integration makes FINAL the runtime.
3. PL and ES already have live milk defaults for other products (PR-ING-007172, PR-ING-007173); the v12
   package does not replace them.
4. Three identity merges rest on article codes, not GTINs (Louis François 450A across FR/GR/QA/OM;
   Valknut VAL444 across CZ/SK) — confirm or veto.
5. Scaffold Technical Bases (169–171) are shown as templates; opening one needs a „choose Main" flow
...
| Item | Result |
|---|---|
| current staging base SHA | branch base `c65e52ef`; staging now `0b42b206` |
| source recipe workbook | GELLATTI_RECEPTURY.xlsx (`a85e32a4…`) |
| final mapper_basement SHA | `a6a849a5…` (2541 × 62) |
...
| canonical-name reconciliation | PASS (634 lines / 16 PI on their PI; UI test proves runtime name by PI) |
...
| Recipe nav order | PASS |
| Inspiracje removed | PASS |
| Udostępnione rename | PASS |
| 177/177 recipes · 1510/1510 lines · 177/177 image map · collection counts | PASS · PASS · PASS · PASS |
| country resolver handoff | PASS (tests); served check pending merge |
| BRAK no-guess | PASS |
| Technical Bases | PASS |
| immutable official recipes | PASS |
| focused tests | PASS (108/108; country 18/18; guards 51 files / 667 tests) |
| typecheck · lint · build · verify:staging | PASS · PASS (0 errors) · PASS · PASS (contracts 225/225) |
| full suite | PASS on `3f2bb1e1` — 1135 files / 14 131 tests, 0 failed; CI re-runs it on the PR head |
| merge SHA / served staging SHA | not merged (§20 gate) / — |
...
| READY FOR OWNER TEST | NO — waits for the §20 merge gate |
