...
No copied formulas in React components. No hidden calculation helpers in UI. No AI-generated recipe numbers.

## Correction boundary

Corrections must come from `proposeCorrections`.

Demo sessions must call the solver with `redact: true`.
...
Use `PlanGate` for gated Pro functionality. Locked Pro children should not be mounted in demo.

Internal Pro/test mode may call `redact: false`.
...
Prefer central copy files such as `src/copy/en.ts` if present. Do not scatter important product copy across components.

## Final rule

When UI quality conflicts with engine safety, engine safety wins. When generic frontend design advice conflicts with this skill, this skill wins.
