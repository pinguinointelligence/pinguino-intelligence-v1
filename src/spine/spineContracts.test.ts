...
describe('Spine boundary — the foundation layer stays pure (static)', () => {
  ...
  it('covers the slice modules', () => {
    expect(sourceFiles.sort()).toEqual([
      'access.ts',
      'baseEngineMetricsAdapter.ts',
      'batchRescueRouter.ts',
      'designRecipe.ts',
      'evaluateTemperatureRegulator.ts',
      'index.ts',
      'integrationFlowDispatch.ts',
      'integrationFlowRouter.ts',
      'normalizeProductProfile.ts',
      'normalizeRecipeIntent.ts',
      'optimizationFlowRouter.ts',
      'optimizationRerunPreview.ts',
      'productProfiles.ts',
      'stockShortageRouter.ts',
