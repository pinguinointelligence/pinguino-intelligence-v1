...
describe('precision + regression', () => {
  it.each(['mleko', 'milk', 'Milch', 'leche', 'latte'])(
    'maps the canonical milk intent "%s" to the same search family',
    (query) => {
      expect(haystackMatchesQuery(norm('MILK 3.6 dairy milk'), query)).toBe(true);
    },
  );
