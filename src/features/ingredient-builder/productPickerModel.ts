...
): string => `Produkt ${hit.displayName} · ID ${hit.id} · wersja ${hit.currentVersionId ?? 'brak'} · Mapper ${hit.mappedIngredientId ?? 'brak'} · moduł ${scope === 'BASE_FORMULATION' ? 'BASE_RECIPE' : 'TOPPING'}`;
...
export function productPickerVerificationView(
  ...
  if (hit.entityKind === 'pi_base') {
    if (hit.currentVersionId === null || hit.currentVersionId === undefined) {
      return {
        ...
        reason: exactPickerBlock(hit, scope, 'currentVersionId', 'Odśwież produkt i utwórz aktualną wersję.'),
      };
    }
    if (!hit.mappedIngredientId) {
      return {
        status: 'MAPPER BINDING REQUIRED',
        reason: exactPickerBlock(hit, scope, 'mappedIngredientId', 'Wybierz dokładne powiązanie Mapper.'),
      };
    }
    ...
    if (!usable) {
      ...
      return {
        ...
        reason: approvalMissing
          ...
          : exactPickerBlock(hit, scope, 'module eligibility', hit.blockedReason
    ...
    if (hit.verificationMethod === 'mapper_estimated') {
      return {
        ...
        reason: 'Produkt ma techniczne dane Mappera, ale źródło nie zostało jeszcze zweryfikowane na podstawie aktualnej etykiety.',
      };
    }
    if (hit.verificationMethod === 'mapper_needs_label_review') {
      return {
        status: 'WYMAGA SPRAWDZENIA ETYKIETY',
        reason: 'Status źródła wymaga sprawdzenia etykiety. Produkt pozostaje dostępny technicznie.',
      };
    }
    if (hit.verificationMethod === 'mapper_other') {
      return { status: 'SYSTEM — DOPASOWANY', reason: 'Źródło produktu pozostaje informacją; użycie techniczne wynika z danych Mappera.' };
    }
  ...
  if (scope === 'POST_PROCESS_ADDON') {
    ...
    if (hit.verificationMethod === 'mapper_estimated') {
      return {
        status: 'Dane szacowane',
        reason: 'Produkt ma techniczne dane Mappera, ale źródło pozostaje szacowane. Etykieta końcowa może nadal wymagać uzupełnienia.',
      };
    }
    if (hit.verificationMethod === 'mapper_needs_label_review' || hit.status === 'blocked'
        || hit.missingFields.length > 0 || hit.invalidFields.length > 0) {
      return {
        status: 'WYMAGA SPRAWDZENIA ETYKIETY',
        reason: 'Stan etykiety pozostaje informacją i nie blokuje technicznego Toppingu z dokładnym Mapperem; Label może pozostać zablokowany.',
      };
    }
  ...
  if (!hit.mappedIngredientId) {
    return {
      status: 'MAPPER BINDING REQUIRED',
      reason:
        exactPickerBlock(hit, scope, 'mappedIngredientId', 'Wybierz dokładne powiązanie Mapper.'),
    };
  }
  if (!hit.usableInBase) {
    return {
      status: 'PRODUCT DATA INCOMPLETE',
      reason: exactPickerBlock(hit, scope, 'approved_for_base / profile eligibility', hit.blockedReason
...
export function productPickerUnavailableReason(
  ...
  if (hit.blockedReason) {
    return exactPickerBlock(hit, scope, 'server technical gate',
  ...
  if (hit.status === 'blocked') {
    ...
    return defects.length > 0
      ...
      : exactPickerBlock(hit, scope, 'technical eligibility', 'Odśwież dane albo wybierz kwalifikowany produkt.');
  }
  if (scope === 'BASE_FORMULATION') {
    return exactPickerBlock(hit, scope, 'mappedIngredientId', 'Wybierz dokładne mapowanie do PINGÜINO Base.');
  }
  if (hit.invalidFields.includes('nutrition_basis_per_100ml_requires_density_for_gram_topping')) {
    return exactPickerBlock(hit, scope, 'nutrition basis 100 ml / density', 'Dodaj gęstość, aby przeliczyć Topping na gramy.');
  }
  const defects = [...hit.missingFields, ...hit.invalidFields].map(defectLabel);
  return defects.length > 0
    ? exactPickerBlock(hit, scope, defects.join(', '), 'Uzupełnij pełne dane etykiety.')
    : exactPickerBlock(hit, scope, 'nutritionPer100g / ingredients / allergens', 'Uzupełnij wartości odżywcze, składniki i alergeny.');
}
