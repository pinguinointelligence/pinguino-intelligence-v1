...
const CATALOGUE: IngredientSearchRow[] = [
  ...
  row(
    'PI-ING-000285',
    'LACTOSE SKIMMED MILK · Valio Eila PRO Milk',
    'lactose_free_skimmed_milk_powder_valio',
    'dairy',
    'lactose_free_skimmed_milk_powder',
  ),
  ...
  row(
    'PI-ING-000179',
    'CREAM 18% · Piątnica Cream · Chilled · BIO',
    'cream_18_percent_bio',
    'dairy',
    'cream_18_percent',
    'Piątnica',
  ),
  row(
    'PI-ING-000184',
    'AMARETTO · Fabbri Cream · Chilled · 0004306',
    'amaretto_fabbri_cream',
    'dairy',
    'cream',
  ),
  ...
  row(
    'PI-ING-000480',
    'NEUTRO S · Giuso Stabilizer · 00003204',
    'neutro_s_giuso',
    'stabilizer',
    'stabilizer_dextrose_mix',
  ),
  // inulin (4 live hits)
  row('PI-ING-000456', 'INULIN · Specialty', 'inulin', 'specialty', 'specialty_component'),
  row(
    'PI-ING-001374',
    'FRIMULSION FIB · Tate & Lyle Fiber',
    'frimulsion_fib',
    'fiber',
    'inulin_oligofructose_blend',
  ),
  ...
  row(
    'PI-ING-000467',
    'FRI STAB LN2 · Tate & Lyle Stabilizer',
    'fri_stab_ln2',
    'stabilizer',
    'stabilizer_blend',
  ),
  ...
  row(
    ...
    'frozen_strawberries',
    'fruit',
    'frozen_fruit_profile',
  ),
  row(
    'PI-ING-001435',
    'STRAWBERRY PUR KERRY · Ravifruit Puree · Frozen · 20374231',
    'puree_strawberry_pur_kerry_ravifruit_20374231',
    'fruit',
    'sweetened_fruit_puree',
  ),
  row(
    'PI-ING-000960',
    'FRAGOLA TRUSKAWKA SORBETTO TOP · Aromitalia Paste · 2867C',
    'pasta_fragola_truskawka_per_sorbetto_top_aromitalia_2867c',
    'flavor_paste',
    'flavored_ice_cream_paste',
  ),
  row(
    'PI-ING-000986',
    'UVA FRAGOLA TRUSKAWKA · Aromitalia Base Mix · 3380',
    'pronto_uva_fragola_truskawka_aromitalia_3380',
    'base_mix',
    'powdered_ice_cream_mix',
  ),
  ...
  row(
    'PI-ING-001707',
    'BANANA · Backaldrin Paste · 873099',
    'banana_backaldrin_paste',
    'flavor_paste',
    'banana_paste',
  ),
  // basil (1 live hit)
  ...
  row(
    'PI-ING-001111',
    'VANIGLIA · Comprital Specialty · PC636PB / 2022',
    'vaniglia_pasta_giubileo_wanilia_comprital_pc636pb_2022',
    'flavor_paste',
    'flavored_ice_cream_paste',
  ),
  row(
    'PI-ING-000748',
    'GOLDEN · PreGel Paste · ST-25822',
    'pasta_golden_wanilia_pregel_st_25822',
    'flavor_paste',
    'flavored_ice_cream_paste',
  ),
  row(
    'PI-ING-001874',
    'COCA-COLA VANILLA · Beverage',
    'coca_cola_vanilla',
    'beverage',
    'cola_soft_drink',
  ),
...
const search = (rawQuery: string) =>
  rankSearchHits(
    CATALOGUE.filter((r) => matchesServerFilter(r, rawQuery)).map(toSearchHit),
    rawQuery,
  );

const ids = (rawQuery: string) => search(rawQuery).map((h) => h.id);
const groupHeadings = (rawQuery: string) =>
  groupHitsByForm(search(rawQuery)).map((g) => g.headingPl);

describe('stable PI-ING-* identity on every result row', () => {
  it.each(['milk', 'śmietana', 'truskawka', 'wanilia'])(
    'every "%s" hit carries its stable Mapper id',
    (q) => {
      const hits = search(q);
...
describe('1+2. milk / mleko', () => {
  it.each(['milk', 'mleko'])(
    '"%s": canonical dairy milk first; groups render in the fixed order',
    (q) => {
      expect(ids(q).slice(0, 2)).toEqual(
        expect.arrayContaining(['PI-ING-000236', 'PI-ING-000296']),
      );
...
describe('3+4. cream / śmietana', () => {
  it.each(['cream', 'śmietana'])(
    '"%s": chilled dairy creams lead and read „Świeże" — never „Inne"',
    (q) => {
      const hits = search(q);
      ...
      expect(resultRowTextPl(hits.find((h) => h.id === 'PI-ING-000260')!)).toBe(
        'CREAM · Nabiał · Świeże',
      );
      // CREAM 18% (subcategory cream_18_percent — live census) is fresh dairy too
      expect(resultRowTextPl(hits.find((h) => h.id === 'PI-ING-000179')!)).toBe(
        'CREAM 18% · Nabiał · Świeże',
      );
...
describe('7+8. sucrose / sacharoza + dextrose / dekstroza', () => {
  it.each(['sucrose', 'sacharoza'])(
    '"%s": the sweetener rows lead; zero-sugar beverages sink',
    (q) => {
      expect(ids(q)[0]).toBe('PI-ING-000514'); // SUCROSE SUGAR
      expect(resultRowTextPl(search(q)[0]!)).toBe(
        'SUCROSE SUGAR · Cukry i substancje słodzące · Suche',
      );
  ...
  it.each(['dextrose', 'dekstroza'])(
    '"%s": DEXTROSE first, stabilizer dextrose-mix follows',
    (q) => {
...
describe('9. inulin / inulina + stabilizer / tara', () => {
  it.each(['inulin', 'inulina'])(
    '"%s": INULIN leads; fiber blends found; honest „Inne" for specialty_component',
    (q) => {
      ...
      expect(resultRowTextPl(search(q).find((h) => h.id === 'PI-ING-001374')!)).toBe(
        'FRIMULSION FIB · Błonnik · Suche',
      );
    },
  );
  ...
  it('"stabilizer" reaches the stabilizer catalogue (49 live hits) — blends dry, agar a powder', () => {
    ...
    expect(found).toEqual(
      expect.arrayContaining(['PI-ING-000492', 'PI-ING-001344', 'PI-ING-000467', 'PI-ING-000480']),
    );
    expect(resultRowTextPl(search('stabilizer').find((h) => h.id === 'PI-ING-001344')!)).toBe(
      'AGAR · Stabilizatory · Proszek',
    );
...
describe('10. pineapple / ananas (no „ananas" row exists — pure alias reach)', () => {
  it.each(['pineapple', 'ananas', 'ananasa', 'świeży ananas'])(
    '"%s": Fresh Fruit → Puree → Pasty → Napoje',
    (q) => {
...
describe('11. strawberry / truskawka + banana / banan + basil / bazylia', () => {
  it.each(['truskawka', 'truskawki', 'świeże truskawki'])(
    '"%s": fresh strawberries first; Polish display rows found',
    (q) => {
      ...
      expect(groupHeadings(q)).toEqual([
        'Świeże',
        'Mrożone',
        'Puree i przeciery',
        'Pasty',
  ...
  it('"strawberry": exact-prefix match may lead (baseline quality rule), fresh still beats frozen, groups identical', () => {
    ...
    expect(groupHeadings('strawberry')).toEqual([
      'Świeże',
      'Mrożone',
      'Puree i przeciery',
      'Pasty',
...
describe('12. vanilla / wanilia (Polish only inside internal names)', () => {
  it.each(['vanilla', 'wanilia', 'wanilii'])(
    '"%s": VANIGLIA + GOLDEN (internal-name Polish) found; pastes before beverages',
    (q) => {
      const found = ids(q);
      expect(found).toEqual(
        expect.arrayContaining(['PI-ING-001111', 'PI-ING-000748', 'PI-ING-001874']),
      );
...
const DIACRITIC_ROWS: IngredientSearchRow[] = [
  ...
  row(
    'PI-ING-001396',
    'FATTY COTTAGE CHEESE FAT 8% · Piątnica Dairy · Chilled',
    'fatty_cottage_cheese_8_percent_fat',
    'dairy',
  ...
  row(
    'PI-ING-001462',
    'ROSE PETALS RÓŻA · Polska Róża Botanical',
    'rose_petals_in_sugar_polska_r_a',
    'botanical',
    'rose_petals_in_sugar',
    'Polska Róża',
  ),
  row(
    'PI-ING-001614',
    'JÄGERMEISTER · Herbal Liqueur · 35% Vol',
    'jagermeister',
    'alcohol',
    'herbal_liqueur',
    'Jägermeister',
  ...
  row(
    'PI-ING-001748',
    'BACARDÍ SUPERIOR · White Rum · 40% Vol',
    'bacardi_rum_superior_40_percent',
    'alcohol',
    'rum',
    'Bacardí',
  ),
  row(
    'PI-ING-001749',
    'BACARDÍ GOLD · Rum · 40% Vol',
    'bacardi_rum_gold_40_percent',
    'alcohol',
    'rum',
    'Bacardí',
  ),
  row(
    'PI-ING-001750',
    'BACARDÍ BLACK · Dark Rum · 40% Vol',
    'bacardi_rum_premium_black_40_percent',
    'alcohol',
    'rum',
    'Bacardí',
  ),
  row(
    'PI-ING-001751',
    'BACARDÍ COCO · Coconut Rum · 35% Vol',
    'bacardi_coco_35_percent',
    'alcohol',
    'flavoured_rum',
    'Bacardí',
  ),
  row(
    'PI-ING-001752',
    'BACARDÍ SPICED · Spiced Rum · 35% Vol',
    'bacardi_spiced_35_percent',
    'alcohol',
    'spiced_rum',
    'Bacardí',
  ),
  row(
    'PI-ING-001753',
    'BACARDÍ LIMÓN · Citrus Rum · 35% Vol',
    'bacardi_limon_35_percent',
    'alcohol',
    'flavoured_rum',
    'Bacardí',
  ),
  row(
    'PI-ING-001754',
    'BACARDÍ RASPBERRY · Raspberry Rum · 35% Vol',
    'bacardi_raspberry_35_percent',
    'alcohol',
    'flavoured_rum',
    'Bacardí',
  ),
  row(
    'PI-ING-001755',
    'BACARDÍ LIME · Lime Rum · 35% Vol',
    'bacardi_lime_35_percent',
    'alcohol',
    'flavoured_rum',
    'Bacardí',
  ),
  row(
    'PI-ING-001756',
    'BACARDÍ TROPICAL · Tropical Rum · 35% Vol',
    'bacardi_tropical_35_percent',
    'alcohol',
    'flavoured_rum',
    'Bacardí',
  ),
  row(
    'PI-ING-001757',
    'BACARDÍ MANGO CHILE · Mango Chile Rum · 35% Vol',
    'bacardi_mango_chile_35_percent',
    'alcohol',
    'flavoured_rum',
    'Bacardí',
  ),
  row(
    'PI-ING-001761',
    'PATRÓN SILVER · 100% Agave Tequila · 40% Vol',
    'patron_silver_tequila_40_percent',
    'alcohol',
    'tequila',
    'Patrón',
  ),
  row(
    'PI-ING-001762',
    'KAHLÚA ORIGINAL · Coffee Liqueur · 16% Vol',
    'kahlua_original_coffee_liqueur_16_percent',
    'alcohol',
    'coffee_liqueur',
    'Kahlúa',
  ),
  row(
    'PI-ING-001779',
    'BÉNÉDICTINE D.O.M. · Herbal Liqueur · 40% Vol',
    'benedictine_dom_liqueur_40_percent',
    'alcohol',
    'herbal_liqueur',
    'Bénédictine',
  ),
  row(
    'PI-ING-001780',
    'B&B BÉNÉDICTINE · Brandy Herbal Liqueur · 40% Vol',
    'benedictine_b_and_b_40_percent',
    'alcohol',
    'herbal_liqueur',
    'Bénédictine',
  ),
