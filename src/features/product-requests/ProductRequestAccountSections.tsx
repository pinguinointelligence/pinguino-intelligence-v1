...
const fileEligible = new Set([
  'FRONT_PHOTO', 'BARCODE_OR_EAN', 'INGREDIENTS', 'NUTRITION_TABLE',
  'ALLERGEN_INFORMATION', 'TECHNICAL_DOCUMENT', 'OTHER',
]);
...
function RequestResponse({ request }: { request: MyProductRequest }) {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (suppliedFields.length === 0) throw new Error('Uzupełnij co najmniej jedną wskazaną pozycję.');
      await resubmitProductRequest({
        requestId: request.id,
        suppliedFields,
        corrections: Object.fromEntries(
          suppliedFields.flatMap((field) => corrections[field]?.trim()
            ? [[field, corrections[field].trim()]]
            : []),
        ),
        files: suppliedFields.flatMap((field) => files[field]
          ? [{ fieldType: field, file: files[field] as File }]
          : []),
      });
    },
    onSuccess: async () => {
    ...
    >
      ...
      <div className="mt-4 grid gap-5 md:grid-cols-2">
        {requested.map((field) => (
          <fieldset key={field.id} className="min-w-0 border border-ink/10 bg-white p-4">
            ...
            {field.instruction ? <p className="mb-3 text-xs text-stone-500">{field.instruction}</p> : null}
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        ...
        <p className="max-w-2xl text-xs leading-relaxed text-stone-500">
          Wpisane dane są dowodem do weryfikacji. Nie stają się automatycznie zaufaną
          informacją o produkcie.
        </p>
      </div>
      {mutation.isError ? (
        <p className="mt-3 text-xs text-status-error" role="alert">{mutation.error.message}</p>
      ) : null}
    </form>
  );
}
...
function RequestRow({ request, archived }: { request: MyProductRequest; archived: boolean }) {
  ...
  const action = useMutation({
    mutationFn: async (kind: 'ARCHIVE' | 'REOPEN' | 'CANCEL') => {
      if (kind === 'CANCEL' && !window.confirm('Anulować zgłoszenie? Tej operacji nie można cofnąć.')) return;
      ...
      >
        <span className="min-w-0">
          ...
          <span className="mt-1 block font-mono text-[11px] text-stone-500">
            #{request.requestNumber} · {request.status} · {new Date(request.updatedAt).toLocaleDateString('pl-PL')}
          </span>
        </span>
      ...
      {expanded ? (
        <div className="mt-4">
          <dl className="grid gap-x-6 gap-y-3 text-xs sm:grid-cols-3">
            <div><dt className="text-stone-500">EAN</dt><dd className="mt-1 font-mono text-ink">{request.ean ?? '—'}</dd></div>
            <div><dt className="text-stone-500">Rynek</dt><dd className="mt-1 text-ink">{request.marketCountryCode ?? '—'}</dd></div>
            <div><dt className="text-stone-500">Marka</dt><dd className="mt-1 text-ink">{request.brand ?? '—'}</dd></div>
          </dl>
          {request.rejectionReason ? <p className="mt-4 text-sm text-status-error">{request.rejectionReason}</p> : null}
          <RequestResponse request={request} />
          <div className="mt-5 flex flex-wrap gap-2">
            {archived ? (
              <Button variant="ghost" size="sm" onClick={() => action.mutate('REOPEN')}>Przywróć</Button>
            ) : (
              <Button variant="ghost" size="sm" onClick={() => action.mutate('ARCHIVE')}>Odłóż do archiwum</Button>
            )}
            {!terminal.has(request.status) ? (
              <Button variant="ghost" size="sm" onClick={() => action.mutate('CANCEL')}>Anuluj zgłoszenie</Button>
            ) : null}
          ...
          {action.isError ? <p className="mt-3 text-xs text-status-error">{action.error.message}</p> : null}
        </div>
      ) : null}
    </article>
  );
...
export function ProductRequestAccountSections() {
  ...
  return (
    <section className="py-5" aria-labelledby="product-requests-heading">
      <h2 id="product-requests-heading" className="text-sm font-semibold text-ink">Zgłoszenia produktów</h2>
      <p className="mt-1 max-w-xl text-xs leading-relaxed text-stone-600">
        Zgłoszenie nie jest składnikiem. Produkt staje się dostępny dopiero po zatwierdzeniu przez Gellatti.
      </p>
      ...
      <div className="mt-5 grid gap-8 xl:grid-cols-3">
        <div>
          ...
          <div className="divide-y divide-ink/10">
            {sortedActive.map((request) => <RequestRow key={request.id} request={request} archived={false} />)}
            {!active.isPending && sortedActive.length === 0 ? <p className="py-5 text-sm text-stone-500">Brak aktywnych zgłoszeń.</p> : null}
          </div>
        </div>
        <div>
          <h3 className="border-b border-ink/10 pb-3 text-xs font-semibold uppercase tracking-[0.12em] text-stone-500">
          ...
          <div className="divide-y divide-ink/10">
            {(archived.data ?? []).map((request) => <RequestRow key={request.id} request={request} archived />)}
            {!archived.isPending && (archived.data?.length ?? 0) === 0 ? <p className="py-5 text-sm text-stone-500">Archiwum jest puste.</p> : null}
          </div>
        </div>
        <div>
          <h3 className="border-b border-ink/10 pb-3 text-xs font-semibold uppercase tracking-[0.12em] text-stone-500">
          ...
          <div className="divide-y divide-ink/10">
            {(contributed.data ?? []).map((product) => (
              <Link key={product.requestId} to={`/products?product=${product.productId}`} className={cn('block py-5', 'pro-focus-ring')}>
                <strong className="block text-sm text-ink">{product.name}</strong>
                <span className="mt-1 block font-mono text-[11px] text-stone-500">{product.productCode}</span>
              </Link>
            ))}
            {!contributed.isPending && (contributed.data?.length ?? 0) === 0 ? <p className="py-5 text-sm text-stone-500">Jeszcze żadnego.</p> : null}
          </div>
        </div>
      </div>
      {(active.isError || archived.isError || contributed.isError) ? (
        <p className="mt-4 text-xs text-status-error" role="alert">Nie udało się odczytać zgłoszeń.</p>
      ) : null}
    </section>
  );
}
