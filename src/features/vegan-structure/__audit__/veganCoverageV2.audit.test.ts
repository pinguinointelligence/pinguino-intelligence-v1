 ...
 * classifier over all 2147 owner-approved Mapper rows. It reads the CSV; it never
...
describe('Vegan v2 Mapper coverage audit (read-only)', () => {
  ...
  it('recomputes current Vegan eligibility counts from the real Mapper base', () => {
    ...
    expect(Object.values(counts).reduce((a, b) => a + b, 0)).toBe(2147);
