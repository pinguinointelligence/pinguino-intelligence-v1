...
describe('canonical product root forward migration', () => {
  ...
  it('creates the six canonical dependent models with private overlays separated', () => {
    ...
    const versions = sql.slice(
      sql.indexOf('create table public.product_versions'),
      sql.indexOf('create table public.product_behavior_bindings'),
    );
    expect(versions).not.toMatch(/private_price|supplier|notes|stock/);
    expect(sql).toContain('user_product_relations_own');
    expect(sql).toContain('user_id=auth.uid()');
    expect(sql).toContain('public.can_use_product_relation_v1(auth.uid(),product_id)');
  ...
  it('turns the former catalog roots into read-only compatibility views', () => {
    ...
    expect(sql).toContain("'{}'::jsonb evidence_snapshot");
    expect(sql).toContain("'{}'::text[] image_phashes");
    const compatibility = sql.slice(sql.indexOf('create view public.global_catalog_products'));
    expect(compatibility).toContain("p.visibility='shared'");
    expect(compatibility).toContain("p.canonical_verification_status<>'blocked'");
    expect(compatibility).not.toContain('select * from public.product_aliases');
    expect(compatibility).not.toContain('select * from public.product_retailer_offers');
    expect(sql).toContain('global_catalog_products_archive_readonly');
    expect(sql).toContain(
      'revoke execute on function public.submit_owned_product_to_global_catalog_v2',
    );
  });

  it('replaces search with canonical versions, behavior and private relations', () => {
    const replacement = sql.slice(
      sql.lastIndexOf('drop function if exists public.search_global_catalog'),
    );
  ...
  it('hydrates shared ingest results through an account-safe projection', () => {
    const projection = sql.slice(
      sql.indexOf('create or replace function public.get_canonical_product_for_account_v1'),
      sql.indexOf('create or replace function public.canonical_product_write_guard'),
    );
    ...
    for (const field of [
      ...
      expect(projection).toContain(`'${field}',null`);
    }
    expect(projection).toContain('left join public.user_product_relations r');
    expect(projection).toContain('to authenticated,service_role');
  });
  ...
  it('provides one service-role-only transaction with idempotency, rate limits, duplicates and atomic outputs', () => {
    ...
    expect(sql).not.toMatch(
      /grant execute on function public\.ingest_product_v1[^;]+authenticated/i,
    );
    expect(sql).toContain('create or replace function public.preflight_product_ingest_v1');
    expect(sql).toContain("'completedResult',v_completed");
    expect(sql).toContain("raise exception 'valid preprocessing rate reservation required'");
    expect(sql).toContain(
      "raise exception 'privileged product source requires an active administrator'",
    );
    expect(sql).toContain("'canonical-ingest-v2:'||left(v_payload_fingerprint,24)");
  });
  ...
  it('routes lifecycle decisions through an admin-only audited branch', () => {
    ...
    expect(sql).toContain(
      "if not v_is_admin then raise exception 'administrator lifecycle decision required'",
    );
    expect(sql).toContain("'admin_lifecycle'");
    expect(sql).toContain(
      'PI Verified lifecycle decision requires independent provenance and clean red flags',
    );
  ...
  it('keeps Mapper authorization version-bound and administrator-only', () => {
    ...
    expect(sql).toContain(
      "if not v_is_admin then raise exception 'administrator Mapper decision required'",
    );
  ...
  it('locks correction targets and never trusts adapter identity keys over canonical keys', () => {
    ...
    expect(sql).toContain('Public version facts are an explicit server allowlist');
    expect(sql).not.toContain("v_facts:=(coalesce(p_input->'facts','{}'::jsonb)");
    expect(sql).not.toContain("'referencePrice',p_input#>'{facts,referencePrice}'");
    expect(sql).not.toContain("'technicalComposition',p_input#>'{facts,technicalComposition}'");
    expect(sql).toContain(
      "raise exception 'duplicate product decision does not match canonical identity'",
    );
    expect(sql).toContain(
      'p.owning_account_id=p_actor_user_id or p.created_by=p_actor_user_id or v_is_admin',
    );
    expect(sql).toContain(
      'v_existing.owning_account_id=p_actor_user_id or v_existing.created_by=p_actor_user_id',
    );
    expect(sql).not.toContain(
      "v_existing.visibility='shared' and v_existing.canonical_verification_status='blocked' and exists(",
    );
    expect(sql).not.toContain(
  ...
  it('preserves PI Base relationships while commercial overlays have one owner table', () => {
    ...
    expect(sql).not.toContain(
      'revoke insert,update,delete on public.global_catalog_favorites,public.global_catalog_recent_usage',
    );
  });
  ...
  it('never promotes OCR to GREEN without an exact server attestation', () => {
    ...
    expect(sql).toContain('cardinality(a.image_checksums)=(select count(*)');
    expect(sql).toContain(
      'i.checksum_sha256 is null or not i.checksum_sha256=any(a.image_checksums)',
    );
  ...
  it('rejects malformed optional nutrition and ignores blank quantity placeholders', () => {
    expect(sql).toContain("nullif(trim(coalesce(v_facts->>'packageSize','')),'') is not null");
    expect(sql).toContain("array['saturatedFat','sugars','fibre']");
    expect(sql).toContain("'nutrition_'||lower(v_unit)||'_type'");
  ...
  it('orders 10300 before a canonical-only 10400 classifier', () => {
    ...
    expect(classifierSql).not.toMatch(
      /(?:insert\s+into|update|delete\s+from|trigger[^;]*on)\s+public\.global_catalog_products\b/i,
    );
    expect(classifierSql).not.toMatch(
      /(?:insert\s+into|update|delete\s+from|trigger[^;]*on)\s+public\.global_catalog_product_versions\b/i,
    );
    expect(classifierSql).not.toMatch(
      /(?:insert\s+into|update|delete\s+from)\s+public\.catalog_product_behavior_bindings\b/i,
    );
