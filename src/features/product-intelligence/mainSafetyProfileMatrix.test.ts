...
const PROFILES: readonly Profile[] = [
  { name: 'GELATO', category: 'milk_gelato', policyId: 'main-berry-fresh-dairy',
    ecoFloorPercent: 25, optimalCeilingPercent: 35, hardLimitPercent: 45,
    liquidDairyCarrierFloorPercent: 30, requiresLiquidDairyCarrier: true },
  { name: 'SORBET', category: 'sorbet', policyId: 'main-sorbet-strawberry-fresh-1553',
    ecoFloorPercent: 60, optimalCeilingPercent: 60, hardLimitPercent: 60,
    liquidDairyCarrierFloorPercent: null, requiresLiquidDairyCarrier: false },
  { name: 'VEGAN', category: 'vegan_gelato', policyId: 'main-vegan-strawberry-fresh-1553',
    ecoFloorPercent: 30, optimalCeilingPercent: 74.7, hardLimitPercent: 74.7,
    liquidDairyCarrierFloorPercent: null, requiresLiquidDairyCarrier: false },
  { name: 'PROTEIN', category: 'protein_gelato', policyId: 'main-protein-strawberry-1553',
    ecoFloorPercent: 10, optimalCeilingPercent: 49.5, hardLimitPercent: 49.5,
    liquidDairyCarrierFloorPercent: null, requiresLiquidDairyCarrier: false },
];

const snapshotFor = (p: Profile, over: Partial<ProductBehaviorSnapshot> = {}): ProductBehaviorSnapshot =>
  ({
    lineId: 'main', resolutionState: 'RESOLVED', processScope: 'BASE_FORMULATION',
    moduleEligibility: { BASE_RECIPE: 'eligible', OPTIMAL: 'eligible', ECO: 'eligible' },
    mainCapability: 'MAIN_CAPABLE', behaviorRole: 'MAIN_PROFILE_SPECIFIC',
    mainClassification: 'MAIN_PROFILE_SPECIFIC',
    mainPolicyId: p.policyId, mainPolicyVersion: '2',
    mainCalibrationLevel: 'EXACT_PRODUCT', mainEquivalentFactor: 1, mainBasis: 'FRUIT_EQUIVALENT',
    familyId: 'fruit', subfamilyId: 'berry', formId: 'fresh',
...
const support = (p: Profile, lineId: string, carrier: boolean): ProductBehaviorSnapshot =>
  snapshotFor(p, { lineId, mainCapability: 'MAIN_TECHNICAL_BLOCKED', behaviorRole: 'STANDARD_ONLY',
    mainClassification: 'STANDARD_ONLY', approvedLiquidDairyCarrier: carrier,
    requiresLiquidDairyCarrier: false } as Partial<ProductBehaviorSnapshot>);

const scene = (p: Profile, mainG: number, carrierG: number, lock: 'main' | 'unlocked') => ({
  recipe: {
    category: p.category, target_batch_grams: BATCH, goals: { formulation_strategy: 'optimal' },
    items: [
    ...
    ].map((i) => ({ ...i, actual_grams: null,
      ingredient: { id: `PI-${i.id}`, name: i.id, category: 'fruit' } })),
  } as unknown as RecipeInput,
  snapshots: { main: snapshotFor(p), carrier: support(p, 'carrier', true), rest: support(p, 'rest', false) },
});

const codes = (v: ReturnType<typeof verifyMainEnvelope>) => (v.ok ? [] : v.violations.map((x) => x.code));
...
describe.each(PROFILES)('Main safety authority — $name', (p) => {
  ...
  it('above the published hard limit an UNCROWNED Main is refused', () => {
    ...
    expect(codes(verifyMainEnvelope({ recipe, snapshots, mode: 'optimal' })))
      .toContain('main_above_hard_limit');
  });

  it('the hard limit binds in ECO as well as OPTIMAL', () => {
    const { recipe, snapshots } = scene(p, overHard, okCarrier(p), 'unlocked');
    for (const mode of ['optimal', 'eco'] as const)
      expect(codes(verifyMainEnvelope({ recipe, snapshots, mode })), mode)
        .toContain('main_above_hard_limit');
  });
  ...
  it(`carrier floor ${p.requiresLiquidDairyCarrier ? 'binds' : 'cannot fire'} for this profile`, () => {
    ...
    const found = codes(verifyMainEnvelope({ recipe, snapshots, mode: 'optimal' }))
      .includes('liquid_dairy_carrier_below_floor');
  ...
  it('the Crown search frontier is the HARD limit, never the preference target', () => {
    ...
    expect(mainEnvelopeSearchCeilingGrams({ recipe, snapshots }))
      .toBeCloseTo((BATCH * p.hardLimitPercent) / 100, 6);
  });
  ...
  it('the preference ceiling stays opt-in and never invalidates an active Crown', () => {
    ...
    expect(codes(verifyMainEnvelope({ recipe, snapshots, mode: 'optimal' })))
      .not.toContain('main_above_optimal_ceiling');
    expect(codes(verifyMainEnvelope({
      recipe, snapshots, mode: 'optimal', enforceOptimalPreferenceCeiling: true,
...
describe.each(PROFILES)('Multi-Main capability group — $name', (p) => {
  const twoMains = (aG: number, bG: number, secondResolved: boolean) => ({
    ...
    snapshots: {
      ...
      main2: snapshotFor(p, secondResolved
        ? { lineId: 'main2' }
        : ({ lineId: 'main2', mainPolicyId: null, mainPolicyVersion: null } as Partial<ProductBehaviorSnapshot>)),
      carrier: support(p, 'carrier', true), rest: support(p, 'rest', false),
    },
  });

  it('a COMPLETE capability group is judged against the shared multi-Main limit', () => {
    const over = g(p.hardLimitPercent) + 20;
    const { recipe, snapshots } = twoMains(Math.round(over / 2) + 10, Math.round(over / 2) + 10, true);
    expect(codes(verifyMainEnvelope({ recipe, snapshots, mode: 'optimal' })))
      .toContain('main_above_hard_limit');
  });
  ...
  it('an INCOMPLETE group is never judged against one member\'s single-product limit', () => {
    const over = g(p.hardLimitPercent) + 20;
    const { recipe, snapshots } = twoMains(Math.round(over / 2) + 10, Math.round(over / 2) + 10, false);
    expect(codes(verifyMainEnvelope({ recipe, snapshots, mode: 'optimal' })))
      .not.toContain('main_above_hard_limit');
  });
});
