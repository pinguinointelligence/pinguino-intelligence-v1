...
export function renderUkLabel(data: MasterLabelData): string {
  ...
  const alcohol =
    ...
    data.alcoholDeclarationReviewed
      ...
      : '';
  return `<section class="market-renderer uk-renderer ${ppds ? 'ppds' : 'prepacked'}" data-regulatory-renderer="uk-label-v2" data-uk-region="${region}"><header class="identity"><h1>${escapeHtml(product)}</h1><p>${escapeHtml(legalName)}</p>${ppds ? '<strong class="context-mark">PPDS · full ingredients and emphasised allergens</strong>' : ''}</header><p class="ingredients" lang="en"><strong>Ingredients:</strong> ${ingredientDeclarationHtml(data, 'en')}</p><p class="allergens"><strong>Alergeny:</strong> ${escapeHtml(allergenStatement)}</p>${renderEuNutrition(data).replace('Nutrition declaration', 'Typical values')}<div class="same-field-of-vision">${netQuantityHtml(data)}${alcohol}</div>${traceabilityHtml(data)}${storageHtml(data, ['en'])}${businessHtml(data)}<small class="jurisdiction-note">${region === 'NI' ? 'Northern Ireland · NI/EU responsible business address rules' : 'Great Britain · UK responsible business address rules'}</small></section>`;
}
