...
export const allergenStatementText = (data: MasterLabelData): string | null =>
  labelAllergenStatement(data);

export const allergenStatementHtml = (data: MasterLabelData): string => {
  const statement = allergenStatementText(data);
  return statement
    ? `<p class="allergens"><strong>Alergeny:</strong> ${escapeHtml(statement)}</p>`
    : '';
};
...
export const netQuantityHtml = (data: MasterLabelData, heading = 'Net quantity'): string => {
  const quantity = packageQuantityForDisplay(data);
  return quantity === '—'
    ? ''
    : `<div class="label-fact net-quantity"><span>${escapeHtml(heading)}</span><strong>${escapeHtml(quantity)}</strong></div>`;
};
...
export const traceabilityHtml = (data: MasterLabelData, bilingual = false): string => {
  ...
  const rows = [
    data.lotCode.trim()
      ? `<div><span>LOT</span><strong data-testid="consumer-lot">${escapeHtml(data.lotCode)}</strong></div>`
      : '',
    data.productionDate.trim()
      ? `<div><span>${bilingual ? 'Production / Production' : 'Production date'}</span><strong>${escapeHtml(data.productionDate)}</strong></div>`
      : '',
    data.dateMark.date
      ? `<div><span>${bilingual ? `${dateHeading} / Date` : dateHeading}</span><strong>${escapeHtml(data.dateMark.date)}</strong></div>`
      : '',
  ].filter(Boolean);
  return rows.length > 0 ? `<div class="traceability">${rows.join('')}</div>` : '';
};
