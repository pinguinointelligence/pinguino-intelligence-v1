import type { MasterLabelData } from '../masterLabel';
import {
  amountPerServing,
  assessCanadaFop,
  percentDailyValue,
  roundCanadaCalories,
  roundCanadaCholesterolMg,
  roundCanadaFatGrams,
  roundCanadaIronMg,
  roundCanadaMg,
  roundCanadaPotassiumCalciumMg,
  roundCanadaProteinGrams,
} from '../regulatoryNutrition';
import {
  allergenStatementHtml,
  businessHtml,
  escapeHtml,
  ingredientDeclarationHtml,
  netQuantityHtml,
  primaryText,
  storageHtml,
  textFor,
  traceabilityHtml,
} from './shared';

const CANADA_DV = Object.freeze({
  fat: 75,
  saturatedAndTrans: 20,
  fibre: 28,
  sugars: 100,
  sodium: 2300,
  potassium: 3400,
  calcium: 1300,
  iron: 18,
});

export function renderCanadianNutritionFacts(data: MasterLabelData): string {
  const source = data.nutritionSource;
  const facts = data.regulatoryNutrition;
  const serving = facts.servingQuantityG;
  if (!source || !serving) return '';
  const amount = (value: number): number => amountPerServing(value, serving) ?? 0;
  const fat = amount(source.fat_g);
  const saturated = source.saturated_fat_g === null ? null : amount(source.saturated_fat_g);
  const trans = facts.transFatGPer100g === null ? null : amount(facts.transFatGPer100g);
  const carbohydrate = amount(source.carbohydrate_g);
  const fibre = source.fiber_g === null ? null : amount(source.fiber_g);
  const sugars = source.sugars_g === null ? null : amount(source.sugars_g);
  const protein = amount(source.protein_g);
  const cholesterol =
    facts.cholesterolMgPer100g === null ? null : amount(facts.cholesterolMgPer100g);
  const sodium = facts.sodiumMgPer100g === null ? null : amount(facts.sodiumMgPer100g);
  const potassium = facts.potassiumMgPer100g === null ? null : amount(facts.potassiumMgPer100g);
  const calcium = facts.calciumMgPer100g === null ? null : amount(facts.calciumMgPer100g);
  const iron = facts.ironMgPer100g === null ? null : amount(facts.ironMgPer100g);
  const row = (label: string, amountText: string, dv?: number, indent = false) =>
    `<tr class="${indent ? 'indent' : ''}"><th>${label} <span>${amountText}</span></th><td>${dv === undefined ? '' : `${dv} %`}</td></tr>`;
  const family = 'bilingual_standard';
  const servingVolume = facts.servingVolumeMl ? ` (${Math.round(facts.servingVolumeMl)} mL)` : '';
  const nutrientRows = [
    row('Fat / Lipides', `${roundCanadaFatGrams(fat)} g`, percentDailyValue(fat, CANADA_DV.fat)),
    saturated !== null
      ? row(
          'Saturated / saturés',
          `${roundCanadaFatGrams(saturated)} g`,
          trans === null
            ? undefined
            : percentDailyValue(saturated + trans, CANADA_DV.saturatedAndTrans),
          true,
        )
      : '',
    trans !== null
      ? row('+ Trans / trans', `${roundCanadaFatGrams(trans)} g`, undefined, true)
      : '',
    row('Carbohydrate / Glucides', `${Math.round(carbohydrate)} g`),
    fibre !== null
      ? row(
          'Fibre / Fibres',
          `${Math.round(fibre)} g`,
          percentDailyValue(fibre, CANADA_DV.fibre),
          true,
        )
      : '',
    sugars !== null
      ? row(
          'Sugars / Sucres',
          `${Math.round(sugars)} g`,
          percentDailyValue(sugars, CANADA_DV.sugars),
          true,
        )
      : '',
    row('Protein / Protéines', `${roundCanadaProteinGrams(protein)} g`),
    cholesterol !== null
      ? row('Cholesterol / Cholestérol', `${roundCanadaCholesterolMg(cholesterol)} mg`)
      : '',
    sodium !== null
      ? row('Sodium', `${roundCanadaMg(sodium)} mg`, percentDailyValue(sodium, CANADA_DV.sodium))
      : '',
    potassium !== null
      ? row(
          'Potassium',
          `${roundCanadaPotassiumCalciumMg(potassium)} mg`,
          percentDailyValue(potassium, CANADA_DV.potassium),
        )
      : '',
    calcium !== null
      ? row(
          'Calcium',
          `${roundCanadaPotassiumCalciumMg(calcium)} mg`,
          percentDailyValue(calcium, CANADA_DV.calcium),
        )
      : '',
    iron !== null
      ? row('Iron / Fer', `${roundCanadaIronMg(iron)} mg`, percentDailyValue(iron, CANADA_DV.iron))
      : '',
  ].join('');
  const servingEn = textFor(facts.servingDescription, 'en');
  const servingFr = textFor(facts.servingDescription, 'fr');
  const servingLine =
    servingEn || servingFr || servingVolume
      ? `<p class="serving">${servingEn ? `Per ${escapeHtml(servingEn)}${servingVolume}` : ''}${servingEn && servingFr ? '<br>' : ''}${servingFr ? `pour ${escapeHtml(servingFr)}${servingVolume}` : ''}</p>`
      : '';
  return `<section class="nutrition-facts canada format-${family}" data-format-family="${family}" data-directory-figure="3.4(B)" data-software-footprint-cm2="29.7"><h2>Nutrition Facts<br><span>Valeur nutritive</span></h2>${servingLine}<div class="calories"><span>Calories</span><strong>${roundCanadaCalories(amount(source.kcal))}</strong></div><p class="daily-value">% Daily Value* / % valeur quotidienne*</p><table><tbody>${nutrientRows}</tbody></table><small>* 5% or less is a little, 15% or more is a lot<br>* 5 % ou moins c'est peu, 15 % ou plus c'est beaucoup</small></section>`;
}

function canadaFopHtml(data: MasterLabelData): string {
  const assessment = assessCanadaFop(data.nutritionSource, data.regulatoryNutrition);
  const assetId = data.regulatoryNutrition.canadaFopAssetId;
  const packageVersion = data.regulatoryNutrition.canadaFopAssetPackageVersion;
  if (assessment.state !== 'required' || !assetId || !packageVersion) return '';
  return `<img class="canada-fop official-authority-asset" src="/regulatory/canada-fop/${encodeURIComponent(assetId)}.svg" alt="Health Canada front-of-package nutrition symbol" data-official-package-version="${escapeHtml(packageVersion)}">`;
}

export function renderCanadaLabel(data: MasterLabelData): string {
  const productEn = primaryText(data.productName, ['en']);
  const productFr = primaryText(data.productName, ['fr']);
  const commonEn = primaryText(data.legalProductName, ['en']);
  const commonFr = primaryText(data.legalProductName, ['fr']);
  const ingredientsEn = ingredientDeclarationHtml(data, 'en');
  const ingredientsFr = ingredientDeclarationHtml(data, 'fr');
  const fop = canadaFopHtml(data);
  const identity =
    productEn || commonEn || productFr || commonFr || fop
      ? `<header class="identity bilingual">${productEn || commonEn ? `<div lang="en">${productEn ? `<h1>${escapeHtml(productEn)}</h1>` : ''}${commonEn ? `<p>${escapeHtml(commonEn)}</p>` : ''}</div>` : ''}${productFr || commonFr ? `<div lang="fr">${productFr ? `<h1>${escapeHtml(productFr)}</h1>` : ''}${commonFr ? `<p>${escapeHtml(commonFr)}</p>` : ''}</div>` : ''}${fop}</header>`
      : '';
  return `<section class="market-renderer canada-renderer" data-regulatory-renderer="canada-nft-v2">${identity}${ingredientsEn ? `<p class="ingredients" lang="en"><strong>Ingredients:</strong> ${ingredientsEn}</p>` : ''}${ingredientsFr ? `<p class="ingredients" lang="fr"><strong>Ingrédients:</strong> ${ingredientsFr}</p>` : ''}${allergenStatementHtml(data)}${renderCanadianNutritionFacts(data)}${netQuantityHtml(data, 'Net quantity / Quantité nette')}${traceabilityHtml(data, true)}${storageHtml(data, ['en', 'fr'], ['Storage', 'Conservation'])}${businessHtml(data, true)}</section>`;
}
