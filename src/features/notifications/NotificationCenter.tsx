...
function UserStartupNotice({ notification }: { notification: DurableNotification }) {
  ...
  return (
    ...
    >
      <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-stone-500">Gellatti · zgłoszenie produktu</p>
      <h2 className="mt-2 text-xl font-semibold tracking-[-0.025em] text-ink">{notification.title}</h2>
      <p className="mt-3 whitespace-pre-line text-sm leading-relaxed text-stone-600">{notification.body}</p>
      <div className="mt-5 flex flex-wrap gap-2">
        {notification.deepLink ? (
          ...
          >
            {needsInfo ? 'Uzupełnij dane' : notification.type === 'PARTNER_ACTIVATED' ? 'Otwórz tryb Partner' : 'Otwórz produkt'}
          </Link>
        ) : null}
        {needsInfo ? (
          <>
            <Button variant="ghost" onClick={() => action.mutate('ARCHIVE')}>Odłóż do archiwum</Button>
            <Button variant="ghost" onClick={() => action.mutate('CANCEL')}>Anuluj zgłoszenie</Button>
          </>
        ) : (
          <Button variant="ghost" onClick={() => action.mutate('ACKNOWLEDGE')}>Rozumiem</Button>
        )}
      </div>
...
function AdminNotifications({ items }: { items: DurableNotification[] }) {
  ...
  const preferenceMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin-notification-preferences'] }),
  });
  ...
  useEffect(() => {
    if (!audioUnlocked || preferences.data?.salesSoundEnabled !== true || pendingSounds.length === 0) return;
    let cancelled = false;
    const play = async () => {
  ...
  return (
    <div className="fixed right-3 top-3 z-[65] sm:right-6 sm:top-6">
      ...
      >
        ...
        {unread ? <span className="absolute -right-1 -top-1 grid min-h-5 min-w-5 place-items-center bg-ink px-1 font-mono text-[10px] text-white">{unread}</span> : null}
      </button>
      {open ? (
        <aside className="mt-2 w-[min(92vw,420px)] border border-ink/15 bg-paper shadow-[0_20px_70px_rgba(16,17,19,0.2)]">
          <header className="border-b border-ink/10 bg-[#f3ede3] p-4">
            <div className="flex items-start justify-between gap-4">
              <div><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-stone-500">Admin</p><h2 className="mt-1 text-base font-semibold text-ink">Centrum powiadomień</h2></div>
            ...
            {items.map((item) => (
              <article key={item.id} className="p-4" data-notification-type={item.type}>
                <div className="flex items-center gap-2">
                  {item.isTest ? <span className="border border-ink/20 px-1.5 py-0.5 font-mono text-[9px]">TEST</span> : null}
                  <time className="font-mono text-[10px] text-stone-500">{new Date(item.createdAt).toLocaleString('pl-PL')}</time>
                </div>
                <strong className="mt-2 block text-sm text-ink">{item.title}</strong>
                <p className="mt-1 whitespace-pre-line text-xs leading-relaxed text-stone-600">{item.body}</p>
                <div className="mt-3 flex gap-2">
                  {item.deepLink ? <Link to={item.deepLink} onClick={() => void notificationAction(item.id, 'READ')} className="pro-focus-ring text-xs font-semibold text-ink underline underline-offset-4">Otwórz</Link> : null}
                  {!item.acknowledgedAt ? <button type="button" onClick={() => void notificationAction(item.id, 'ACKNOWLEDGE').then(() => queryClient.invalidateQueries({ queryKey: ['notifications', true] }))} className="pro-focus-ring text-xs text-stone-500">Potwierdź</button> : null}
                </div>
              </article>
            ))}
            {items.length === 0 ? <p className="p-5 text-sm text-stone-500">Brak powiadomień.</p> : null}
          </div>
        </aside>
      ) : null}
    </div>
