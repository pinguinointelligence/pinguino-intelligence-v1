 ...
 * controls RIGHT. Hover / active state is a soft translucent ivory pill — never
 * an underline. Menus open on hover and focus; the page behind dims + blurs.
 */
export function TopNav({ onNewRecipe }: { onNewRecipe?: () => void }) {
  ...
  // Hover-intent: a short grace period lets the cursor cross the gap into the panel.
  ...
  return (
    <>
      <header className="sticky top-0 z-50 flex h-16 items-center justify-between bg-shell px-6 text-ivory">
        {/* Left — logo */}
        <Link to="/" onClick={closeNow} aria-label={copy.brand.full} className="shrink-0">
        ...
        >
          <nav aria-label={copy.brand.full} className="flex items-center gap-1">
            {NAV_ITEMS.map((item) => {
              const isOpen = openId === item.id;
              const isActive = pathname === item.to;
              return (
                <div key={item.id} className="relative" onMouseEnter={() => open(item.id)}>
                  <Link
                    to={item.to}
        ...
        {/* Right — account (desktop) + menu drawer (mobile) */}
      ...
      {/* Page-behind dim + blur while a menu is open */}
      {openItem?.groups ? (
        <button
          type="button"
          aria-hidden
  ...
  if (authStatus === 'authed' && user) {
    return (
      <button
        ...
        className="hidden rounded-full px-3.5 py-2 text-sm text-ivory/75 transition-colors hover:bg-ivory/10 hover:text-ivory lg:block"
      >
        {copy.nav.signOut}
      </button>
    );
