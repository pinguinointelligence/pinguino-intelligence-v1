...
/**
 * A REFUSED MAIN SWEEP IS NEVER AN ECHO — and never the conflict's evidence
 ...
 *
 * Two served defects, one CORE, both surfaces:
 *  1. `buildOptimizePreview` skipped the Main safety check for ANY exact Direction, so that
 *     vector reached the ProductBehavior binding and the customer was told
 *     „zatwierdzony płynny nośnik mleczny ma 22.8%” (GEL-P0-027: an empty sweep is a
 *     refusal, never an echo);
 *  2. once the refusal was typed, its EVIDENCE still came from that vector: PRO said
 *     „płynna baza ma 20,3% / 15,5%, a wymagane minimum to 30%” about MILK 203 /
 *     STRAWBERRIES 378 — a recipe nobody proposed.
 *
 ...
 * diagnosis's relaxation that keeps the customer's Direction — and no measurement of the
 * diagnostic vector anywhere in the conflict's evidence (CORE result, diagnosis blockers,
 * the PRO gap sentence, the HOME panel).
 *
...
/** The conflict's evidence names none of the diagnostic vector's measurements. */
function expectNoDiagnosticEvidence(text: string, diagnosticPercents: readonly number[]) {
  for (const percent of diagnosticPercents) {
    expect(text).not.toContain(formatPercentPl(percent));
    expect(text).not.toContain(`${percent.toFixed(1)}%`);
  }
}
...
describe('a refused Main sweep ends in the typed lock conflict — CORE', () => {
  for (const { label, batch } of SERVED_CASES) {
    it(`${label}: impossible_under_constraints on the customer's lock, evidence never from the diagnostic vector`, async () => {
      ...
      // The vector CORE's Main safety check measured last is the diagnostic one: the
      // untouched seat Main with every other line scaled around it — nobody proposed it.
      ...
      // None of its verdicts, none of its measurements, is the conflict's evidence.
      for (const violation of diagnostic.violations) {
        expect(result.blockingViolations ?? []).not.toContainEqual(violation);
      }
      expect(result.blockingViolations ?? []).toEqual([]);
      expect(result.residualViolatedMetrics).toEqual([]);
      expect(lockConflictBlockers(result)).toEqual([]);
      expectNoDiagnosticEvidence(JSON.stringify(result), diagnosticPercents);
    });
  }
...
describe('HOME and PRO present the same typed conflict and the same Direction-keeping relaxation', () => {
  for (const { label, batch, moves } of SERVED_CASES) {
    it(`${label}: HOME recalculateHomeRecipe → decision → „Użyj propozycji"; PRO the same`, async () => {
      ...
      // The diagnostic vector's measurements, taken where CORE's Main safety check takes
      // them (the same request HOME's PRZELICZ is about to send).
      addBananaAndKiwi();
      const { input, constraints, options } = coreRequest();
      envelope.recording = true;
      ...
      expect(diagnosticPercents.length).toBeGreaterThan(0);
      expect(home!.conflict.diagnosis.blockers).toEqual([]);
      expectNoDiagnosticEvidence(JSON.stringify(studio().lockConflict), diagnosticPercents);
      expectNoDiagnosticEvidence(JSON.stringify(studio().previewIssue), diagnosticPercents);
      ...
      expectNoDiagnosticEvidence(document.body.textContent ?? '', diagnosticPercents);
      await act(async () =>
        homePanel!
      ...
      expect(pro!.conflict.diagnosis.blockers).toEqual([]);
      expectNoDiagnosticEvidence(JSON.stringify(studio().lockConflict), diagnosticPercents);
      expectNoDiagnosticEvidence(JSON.stringify(studio().previewIssue), diagnosticPercents);

      // PRO's panel (LockConflictPanel, surface="pro"): the gap sentence describes the
      // conflict itself, not the shares of a vector nobody proposed.
      const proPanel = panel('pro');
      expect(proPanel).not.toBeNull();
      expect(proPanel!.dataset.status).toBe('relaxation_found');
      expect(proPanel!.querySelector('[data-testid="lock-conflict-gap"]')?.textContent).toBe(
        constraintStudioCopy.lockConflict.genericGap,
      );
      expect(proPanel!.querySelector('[data-testid="lock-conflict-measures"]')).toBeNull();
      expectNoDiagnosticEvidence(document.body.textContent ?? '', diagnosticPercents);
      await act(async () =>
