...
/**
 * ONE requested Direction axis, expressed so this module keeps NO Direction
 * dependency (owner P1-A kept the same boundary for `directionOnlyResidual`).
 *
 * `read` is the ENGINE value of the axis metric for a candidate and `target` is
 * the value the caller's own Direction plan asks for. Nothing here knows what a
 * band, a level or a profile is: the caller owns the plan, this module owns the
 * arithmetic that aims a gram move at the number the caller supplied.
 */
export interface ExactDirectionAxisTarget {
  read: (candidate: RecipeInput) => number;
  target: number;
}
...
export interface DraftSweepArgs {
  ...
  /**
   * THE REQUESTED DIRECTION AXES, as plain (read, target) pairs. Supplied only
   * when an exact Direction objective is active. Absent ⇒ the exact-target
   * stage never runs and this module behaves exactly as it did before.
   */
  exactDirectionTargets?: readonly ExactDirectionAxisTarget[];
  /**
   * §12 — may the exact-target stage enter the DEVIATING region of the ladder?
   *
   * Absent/false ⇒ preserving rungs only, so the stage can never collapse a
   * positive user line. The caller sets it only for a second, explicitly
   * preserve-first pass, and may then keep the result ONLY when it fully
   * reaches the requested target — the same „deviate only when proven
   * necessary" rule the single-line sweep already applies.
   */
   ...
   * keeps the in-solver use byte-identical. `'nearest'` is for the caller that
   * is choosing what to SHOW: there the question is not „is this another
   * improving solver move" but „is this the nearest legal candidate", so the
   * smallest distance wins outright. It is only sound together with `accept`,
   * which is what guarantees the candidate is legal in the first place.
   */
  acceptanceRule?: 'strictly_better' | 'nearest';
}
...
export function sweepDraftCandidateVector(args: DraftSweepArgs): DraftSweepResult | null {
  ...
  // ── EXACT-TARGET STAGE ────────────────────────────────────────────────────
  // LAST, and only on a Direction request: both ladder passes above have now
  // failed, so nothing this stage returns can change a trajectory either of
  // them could already take. It is the difference between „no rung of the
  // ladder happened to improve this" and „no candidate aimed at the requested
  // target is legal" — and only the second is an honest INFEASIBLE.
  const aimed = sweepExactDirectionTarget(args, state, best);
  if (aimed !== null && materiallyBetter(aimed.measure, args.startMeasure)) {
    return {
      input: aimed.input,
      measure: aimed.measure,
      moves: [...moves, ...aimed.moves],
      materialUserIntentDeviation: materialDeviationTaken,
    };
  }
...
/**
 * Cost ceiling for ONE exact-target pass. Orchestration only — not a band, not
 * science. It bounds the pass at a fixed number of engine-priced candidates so
 * a Direction round can never grow with recipe size.
 */
export const EXACT_DIRECTION_EVALUATION_BUDGET = 400;

/** Smallest engine response worth solving against (guards a singular system). */
const EXACT_DIRECTION_RESPONSE_EPS = 1e-9;

/**
 * ── THE EXACT-TARGET STAGE (P1 false-infeasible closure) ───────────────────
 *
 * THE DEFECT THIS CLOSES
 * ----------------------
 * Every move the two passes above can make is a rung of a FIXED RELATIVE
 * LADDER, accepted only when it strictly improves the engine measure. Nothing
 * in this module ever computes WHERE THE REQUESTED TARGET IS, so the search is
 * a strictly-improving local descent: it halts at the first barrier and the
 * pipeline reports that halt to the customer as „this level cannot be reached"
 * or as „the nearest possible result". Measured on the served tree:
 *
 *   · HOME gelato + 507 g strawberries, Sweetness −1 → halts at POD 16.050
 *     while an engine-clean whole-gram candidate exists at POD 13.98
 *     (the customer reaches it by tapping „Mniej słodkie" once more);
 *   · butter-pecan with one 80 g sugar lock, Sweetness +1 → halts 0.158 below
 *     the band while a clean candidate exists inside it;
 *   · blood-orange sorbet → the halt point is presented as „najbliższy" and is
 *     26×–156× farther from the target than a candidate in the same space;
 *   · the same halt for −1 and for −2 → two different requests, one answer.
 *
 * WHAT THIS STAGE IS (and is NOT)
 * -------------------------------
 * It is ORCHESTRATION and arithmetic. It reads NO band, NO level, NO profile
 * and NO coefficient: the caller hands it `(read, target)` pairs taken from the
 * caller's own Direction plan, and every candidate it builds is applied through
 * the engine's OWN `applyCorrectionActions` and judged by the caller's OWN
 * `measure`. It invents nothing the engine did not say.
 *
 * HOW
 * ---
 * At fixed total mass each axis metric responds AFFINELY to a mass-neutral gram
 * transfer, so the engine's own response is measurable with one probe per line:
 * move +1 g onto a line, −1 g off a reference line, read the axes. With that
 * table the stage SOLVES the small linear system whose solution lands the
 * requested axes exactly (one mover for one axis, two movers for two), instead
 * of hoping a rung of the ladder happens to land there.
 *
 * EVERY EXISTING GATE STILL DECIDES.  The admissible space is unchanged: the
 * per-line bounds are the ladder's OWN reach, §17 padlocks, Main, poured
 * actuals and the stabilizer dosage window are re-checked by the same helpers
 * the other passes use, and acceptance is the same `strictlyBetter`.
 *
 * USER INTENT: PRESERVE FIRST. By default the stage never enters the deviating
 * region of the ladder, so it can never collapse a positive user line. The
 * caller may run a second pass with `allowMaterialDeviation` — and, exactly like
 * the single-line sweep above, may keep that result ONLY when it fully REACHES
 * the requested target, never merely to shave a residual. The existing
 * user-intent deviation report and its consent gate then describe it.
 */
function sweepExactDirectionTarget(
  args: DraftSweepArgs,
  ...
  // SCOPE GATE: identical to the paired-exchange pass. Every non-Direction flow
  // keeps its previous candidate set and its previous cost byte-for-byte.
  if (startState.goals?.direction_targets_active !== true) return null;
  const targets = args.exactDirectionTargets;
  if (targets === undefined || targets.length === 0 || targets.length > 2) return null;

  const batch = startState.target_batch_grams;
  const vector = buildDraftCandidateVector(startState, set, excludedIngredientIds);
  if (vector.length < 2) return null;

  /** The PRESERVING reach of each line's own ladder — never the collapse rungs. */
  const boundsFor = (candidate: DraftAdjustmentCandidate): { lo: number; hi: number } | null => {
    const preserving = candidate.testedGrams.filter(
      (grams) =>
        args.allowMaterialDeviation === true ||
        candidate.anchorGrams === null ||
        !isMaterialUserIntentDeviation(candidate.anchorGrams, grams, batch),
    );
    if (preserving.length === 0) return null;
    const lo = Math.min(candidate.currentGrams, ...preserving);
    const hi = candidate.increasable
  ...
  const bounded = vector
    .map((candidate) => ({ candidate, bounds: boundsFor(candidate) }))
    .filter((entry): entry is { candidate: DraftAdjustmentCandidate; bounds: { lo: number; hi: number } } => entry.bounds !== null);
  ...
  /**
   * Apply a whole solved vector of gram moves through the ENGINE's own applier,
   * decreases first so the mass a raise needs is already free. Every engine
   * refusal (held line, poured actuals, over-reduction) and the stabilizer
   * dosage clamp still stop the move — nothing here bypasses them.
   */
  const applySolved = (moves: readonly DraftAdjustmentMove[]): RecipeInput | null => {
    const ordered = [...moves].sort(
      (left, right) => left.toGrams - left.fromGrams - (right.toGrams - right.fromGrams),
    );
    let state = startState;
    for (const move of ordered) {
      const line = state.items.find((item) => item.id === move.lineId);
      if (line === undefined) return null;
      const staged: DraftAdjustmentCandidate = {
        ...boundsById.get(move.lineId)!.candidate,
        currentGrams: line.planned_grams,
      };
      const actions = draftAdjustmentActions(staged, move.toGrams);
      if (actions.length === 0) continue;
      if (violatesInternalStabilizerProfileAuthority(state, actions[0]!)) return null;
      const applied = applyDraftAdjustment(
        state,
        { ...move, fromGrams: line.planned_grams, actions },
        constraints,
      );
  ...
  for (const reference of bounded) {
    ...
    const movers = bounded.filter((entry) => entry.candidate.lineId !== reference.candidate.lineId);
    if (movers.length === 0) continue;

    // THE ENGINE'S OWN RESPONSE: one mass-neutral probe per line.
    const response = new Map<string, number[]>();
    const base = targets.map((axis) => axis.read(startState));
    for (const mover of movers) {
      const probed: RecipeInput = {
        ...startState,
        items: startState.items.map((item) =>
          item.id === mover.candidate.lineId
            ? { ...item, planned_grams: item.planned_grams + 1 }
            : item.id === reference.candidate.lineId
              ? { ...item, planned_grams: item.planned_grams - 1 }
              : item,
        ),
      };
      response.set(
        mover.candidate.lineId,
        targets.map((axis, index) => axis.read(probed) - base[index]!),
      );
    }

    const need = targets.map((axis, index) => axis.target - base[index]!);
    /** Solved gram deltas per line, exact by construction. */
    const solved: Array<Map<string, number>> = [];
    if (targets.length === 1) {
      for (const mover of movers) {
        const gradient = response.get(mover.candidate.lineId)![0]!;
        if (Math.abs(gradient) < EXACT_DIRECTION_RESPONSE_EPS) continue;
        const delta = need[0]! / gradient;
      ...
      for (let i = 0; i < movers.length; i += 1) {
        for (let j = i + 1; j < movers.length; j += 1) {
          ...
          const b = response.get(movers[j]!.candidate.lineId)!;
          const determinant = a[0]! * b[1]! - b[0]! * a[1]!;
          if (Math.abs(determinant) < EXACT_DIRECTION_RESPONSE_EPS) continue;
          const x = (need[0]! * b[1]! - need[1]! * b[0]!) / determinant;
          const y = (a[0]! * need[1]! - a[1]! * need[0]!) / determinant;
          solved.push(
            new Map([
              [movers[i]!.candidate.lineId, x],
              [movers[j]!.candidate.lineId, y],
              [reference.candidate.lineId, -(x + y)],
            ]),
          );
        }
      }
    ...
    for (const deltas of solved) {
      ...
      for (const [lineId, delta] of deltas) {
        if (Math.abs(delta) < MIN_MOVE_GRAMS) continue;
        const entry = boundsById.get(lineId);
        if (entry === undefined) { scale = 0; break; }
        const headroom =
          delta > 0
            ? entry.bounds.hi - entry.candidate.currentGrams
            : entry.candidate.currentGrams - entry.bounds.lo;
        if (headroom <= 0) { scale = 0; break; }
        scale = Math.min(scale, headroom / Math.abs(delta));
      }
      if (scale <= 0) continue;
      // The full (or fully clamped) step, then progressively smaller ones: the
      ...
      for (const fraction of scale === 1 ? [1, 0.5, 0.25] : [scale, scale / 2, scale / 4]) {
        ...
        for (const [lineId, delta] of deltas) {
          ...
          moves.push({
            lineId,
            ingredientId: entry.candidate.ingredientId,
            ingredientName: entry.candidate.ingredientName,
            fromGrams: entry.candidate.currentGrams,
            toGrams,
            direction: toGrams > entry.candidate.currentGrams ? 'increase' : 'decrease',
            actions,
          });
        }
        ...
        // candidate it refuses never ends the search — the next one is tried.
        if (args.accept !== undefined && !args.accept(normalized)) continue;
        const accepted =
          args.acceptanceRule === 'nearest'
            ? next.severityPoints < best.severityPoints - SEVERITY_EPS
            : strictlyBetter(next, best);
        if (!accepted) continue;
        best = next;
        winner = { input: normalized, measure: next, moves };
      }
    }
  }
...
/**
 * The exact-target stage as a STANDALONE candidate generator (P1 — INV-1/INV-2).
 *
 * `sweepDraftCandidateVector` runs it as its last resort inside the solver loop.
 * The pipeline also needs it at the ONE place where the candidate it is about to
 * SHOW is finally chosen — the point where „this level cannot be reached" and
 * „this is the nearest possible result" are decided. Same function, same gates,
 * same engine judge; only the convergence floor differs, because there the
 * question is not „is another solver round worth it" but „is what we are about
 * to present actually the nearest thing we can build".
 */
export function searchExactDirectionTargetCandidate(
  args: DraftSweepArgs,
): DraftSweepResult | null {
  return sweepExactDirectionTarget(args, args.start, args.startMeasure);
}
