...
export const INGREDIENT_INTAKE_COLUMNS: readonly IngredientIntakeColumn[] = [
  ...
  { key: 'approved_for_engines', label: 'Approved for engines', type: 'boolean', required: true, defaultValue: false, allowedValues: BOOL, description: 'Technical gate: core composition and POD/PAC are complete enough for PI. Provenance status and confidence remain separate information.' },
  { key: 'verification_status', label: 'Verification status', type: 'enum', required: true, defaultValue: 'draft', allowedValues: VERIFICATION_STATUSES, description: 'Informational provenance/review state. It never replaces Base or Engine technical approvals.' },
