...
function Overview() {
  ...
  return <><Heading eyebrow="Overview" title="Stan operacyjny" detail="Rzeczy wymagające decyzji, pieniądze i awarie — bez dekoracyjnych wskaźników." />
    ...
    <Link to="/admin/product-requests" className="mt-5 inline-flex min-h-10 items-center border border-ink/15 px-4 text-xs font-semibold text-ink">Otwórz PRODUCT REQUESTS →</Link>
...
function ProductRequests() {
  ...
  if(selected) return selected.requestType==='PRODUCT_CAPABILITY_REANALYSIS'
    ? <AdminProductCapabilityReanalysisDetail request={selected} onClose={()=>navigate(`/admin/product-requests`)} />
    : <RequestDetail request={selected} onClose={()=>navigate(`/admin/product-requests`)} />;
  return <><Heading eyebrow="Catalog intake" title="Product Requests" detail="Zgłoszenie jest dowodem do weryfikacji. Nie istnieje w Pickerze ani Engine przed zatwierdzeniem."/>
    ...
    <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <select aria-label="Request source filter" value={filter.source} onChange={(e)=>setFilter({...filter,source:e.currentTarget.value})} className="min-h-10 border border-ink/15 px-3 text-xs"><option value="">All sources</option><option value="SCANNER">Scanner</option><option value="MANUAL_EVIDENCE">Manual evidence</option><option value="ADMIN">Admin</option><option value="CONTRIBUTOR_REANALYSIS">Ponowna analiza</option></select>
    </div>
    <div className="mt-4 overflow-x-auto"><table className={table}><thead><tr><th className={th}>ID / AGE</th><th className={th}>PRODUCT</th><th className={th}>USER</th><th className={th}>MARKET</th><th className={th}>TYPE / SOURCE</th><th className={th}>STATUS</th><th className={th}>NEXT</th></tr></thead><tbody>{filtered.map((request)=><tr key={request.id}><td className={td}>{request.requestType==='PRODUCT_CAPABILITY_REANALYSIS'?<><strong>Ponowna analiza</strong><br/><span className="font-mono text-[10px] text-stone-400">{request.id}</span></>:<span className="font-mono">#{request.requestNumber}</span>}<br/><span className="text-stone-400">{new Date(request.submittedAt).toLocaleDateString('pl-PL')} · {Math.floor(((query.dataUpdatedAt||Date.parse(request.updatedAt))-Date.parse(request.submittedAt))/86400000)}d</span></td><td className={td}><strong className="text-ink">{request.name??'Nazwa nieustalona'}</strong><br/>{request.brand??'—'} · {request.ean??'brak EAN'}{request.requestType==='PRODUCT_CAPABILITY_REANALYSIS'?<><br/><span className="font-mono text-[10px] text-stone-400">{request.productCode??request.canonicalProductId}</span></>:null}</td><td className={td}>{request.requesterEmail}<br/><span className="font-mono text-[10px] text-stone-400">assigned {request.assignedAdminUserId??'—'}</span></td><td className={td}>{request.marketCountryCode??'—'}<br/><span className="text-stone-400">origin {request.countryOfOrigin??'—'}</span></td><td className={td}>{request.requestType}<br/><span className="text-stone-400">{request.source}</span></td><td className={td}><StatusChip status={statusTone(request.status)}>{request.status}</StatusChip></td><td className={td}><button className="min-h-10 border border-ink/15 px-3 font-semibold text-ink" onClick={()=>navigate(`/admin/product-requests?request=${request.id}`)}>Otwórz →</button></td></tr>)}</tbody></table></div>
...
function RequestDetail({request,onClose}:{request:AdminProductAddRequest;onClose:()=>void}) {
...
function statusTone(status:string):'ideal'|'risky'|'needs_correction'|'good'{if(['APPROVED','ACCEPTED'].includes(status))return'ideal';if(['REJECTED','USER_CANCELED'].includes(status))return'risky';if(['NEEDS_INFO','RESUBMITTED','IN_REVIEW'].includes(status))return'needs_correction';return'good'}
