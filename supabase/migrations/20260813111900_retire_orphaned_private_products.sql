-- An active account-private product must always have an owner. Account
-- deletion preserves its immutable history but atomically retires it when the
-- owning FK is anonymized.
alter table public.products
  drop constraint products_canonical_privacy_check;
alter table public.products
  add constraint products_canonical_privacy_check check (
    (visibility='shared' and owning_account_id is null)
    or (visibility='account_private' and (owning_account_id is not null or not is_active))
...
begin
  ...
  if tg_table_name='products' and tg_op='UPDATE'
    ...
    if v_new->'owning_account_id'='null'::jsonb
      and v_old->'owning_account_id'<>'null'::jsonb then
      new.is_active:=false;
    end if;
    return new;
  end if;
