-- SCAN IMPORT 2.0 — exact resolver: a ready add-on is usable (role readiness, not base physics alone).
--
-- 20260906005305 read the version's `engineUsable` — the BASE-physics verdict, false by design for a
-- TOPPING_ONLY article — so a ready topping never resolved as usable. The physics side of the flag now
-- reads the readiness authority's `gellattiReadiness.ready` where the facts carry it, falling back to
-- `engineUsable`; the binding side (the module the catalogue grants the role) is unchanged.
...
begin
  ...
  select p.id,
    ...
    -- usable = the role's readiness (gellattiReadiness.ready; engineUsable for older versions) AND the
    -- module the catalogue actually grants the role (TOPPING for an add-on, BASE_RECIPE otherwise);
    -- a Mapper reference row is always usable
    (p.product_kind = 'mapper_reference' or (
      coalesce(
        (pv.facts #>> '{productIntelligence,productAccuracyAssessment,gellattiReadiness,ready}')::boolean,
        (pv.facts -> 'productIntelligence' ->> 'engineUsable')::boolean,
        false)
