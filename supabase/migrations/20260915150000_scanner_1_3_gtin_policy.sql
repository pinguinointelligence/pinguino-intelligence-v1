-- Scanner 1.3 — exact resolver fail-closed parity.
--
-- The original resolver accepted GTIN-14 and accepted 8/12-digit values when the
-- symbology argument was omitted. The active Scanner V2 adapter supplies a
-- canonical GTIN-13 plus the actual capture format, so the resolver must expose
-- exactly that contract: explicit EAN/UPC format for captured aliases, and a
-- canonical-only 13-digit query when no format is supplied.
--
-- This is a forward migration rather than an edit to the already-applied 1.2
-- migration. It is read-only and does not modify mapper_basement.
...
comment on function public.resolve_exact_products_by_gtin_v1(text, text) is
  'Scanner 1.3 exact identity: canonical GTIN-13 without a format, or validated EAN/UPC capture alias with explicit symbology; no GTIN-14.';
