 ...
 * and what counts as deleted (`gellatti_share_photo_cleanup_settle_v1`). This
 * module only refuses a malformed claim and shapes the settle call — nothing in
 * it can widen what gets deleted.
 */
 ...
 * as removed. Without an error every planned name is reported, and settle still
...
export function removalOutcome(
  ...
  return { removed: plan.names, error: null };
}

/** Constant-time comparison of the presented bearer with the service role key. */
