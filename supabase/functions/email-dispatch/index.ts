 ...
 * EMAIL_PROVIDER_NAME (optional, default 'resend'), EMAIL_PROVIDER_ENDPOINT
 * (optional — QA capture sink; unset means the real provider),
...
/* THE VENDOR ENDPOINT — one literal, one override.
   The default is the real provider and nothing about delivery changes while
   EMAIL_PROVIDER_ENDPOINT is unset, which is every environment that sends real
   mail. An isolated QA branch sets it to its own capture sink, so the mail lane
   can be exercised end to end — claim, provider answer, settle, retry, dedup —
   without a message ever reaching a real recipient. An absent provider key is
   NOT that isolation: it stops the path before the provider and leaves the
   queue to deliver itself the moment a key appears. */
const DEFAULT_PROVIDER_ENDPOINT = 'https://api.resend.com/emails';
const PROVIDER_ENDPOINT = Deno.env.get('EMAIL_PROVIDER_ENDPOINT')?.trim() || DEFAULT_PROVIDER_ENDPOINT;
