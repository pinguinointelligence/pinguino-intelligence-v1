...
// Modeled on scripts/seed-staging-canonical-country-milk.mjs: product request
// → START_REVIEW → ADMIN_EVIDENCE_PATCH → duplicate preview → catalog-submit
// approval → APPROVE_LINK → canonical slot review → ADD_MARKET → country
// PRIMARY_DEFAULT. It never replaces or deactivates an existing assignment.
...
import {
  APPLY_PROJECT_REF,
  CATALOG_SUBMIT_REVISION,
  buildApplyPlan,
  evaluateApplyGuard,
} from './lib/applyPlan.mjs';

const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), '..', '..');
const DEFAULT_REGISTRY = 'docs/country-products/gelato-base-v12/registry.json';
const MANIFEST = 'docs/country-products/gelato-base-v12/manifest.json';

const guard = evaluateApplyGuard(process.argv.slice(2));
...
const registryPath = resolve(repoRoot, String(guard.args.get('--registry') ?? DEFAULT_REGISTRY));
...
// ----------------------------------------------------------------- APPLY
// Reached only with the exact project ref, --apply and the owner approval token.

const manifest = JSON.parse(readFileSync(resolve(repoRoot, MANIFEST), 'utf8'));
const registryEntry = manifest.outputs.find((entry) => entry.path === DEFAULT_REGISTRY);
...
for (const product of plan.productsToCreate) {
  ...
  if (request.status !== 'APPROVED') {
    ...
    const { data: approved, error: approveError } = await admin.functions.invoke('catalog-submit', {
      body: {
        source: 'admin',
        idempotencyKey: `product-request:${request.id}:${CATALOG_SUBMIT_REVISION}`,
        ...
        distinguishingEvidence: different
          ? { exactGtin: product.gtin, comparedProductId: different.product_id, reason: 'Different GTIN or identity per owner workbook v12.', productKey: product.productKey }
          : {},
      },
    });
  ...
  for (const market of product.additionalMarkets) {
    const { error } = await admin.rpc('gellatti_admin_catalog_action_v1', {
      ...
      p_action: 'ADD_MARKET',
      p_payload: { market, reason: `${product.productKey}: owner workbook v12 route ${market}` },
    });
