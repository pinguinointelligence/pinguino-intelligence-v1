...
// ---------------------------------------------------------------------------
// Owner-approved DISCOVERY links (2026-09-17): a product that the frozen Search source
// deliberately keeps as an exact-only product may still be used to FIND existing recipes
// made with it. This is a THIRD projection with its own source hash and manifest — the
// frozen workbook is not edited, the release bytes are untouched (the supplement is NOT
// part of `sources`, so `authorityHashes` does not change), and the shape cannot express
// a default: the build refuses any link whose product is in that concept's frozen order.
// ---------------------------------------------------------------------------
const CONCEPT_DISCOVERY_SOURCE = path.join(
  ROOT,
  'src/features/mapper-search-runtime/conceptDiscoveryLinks.source.json',
);
const CONCEPT_DISCOVERY_OUT = path.join(
  ROOT,
  'src/features/mapper-search-runtime/generated/conceptDiscoveryLinks.ts',
);
const CONCEPT_DISCOVERY_MANIFEST_OUT = path.join(
  ROOT,
  'src/features/mapper-search-runtime/generated/conceptDiscoveryManifest.ts',
);
const discoverySourceText = fs.readFileSync(CONCEPT_DISCOVERY_SOURCE, 'utf8');
const discoveryInput = JSON.parse(discoverySourceText);
if (
  discoveryInput.authority !== 'HOME_CONCEPT_DISCOVERY_LINKS_OWNER_APPROVED_V1' ||
  discoveryInput.usage !== 'RECIPE_DISCOVERY_ONLY'
) {
  throw new Error('conceptDiscoveryLinks.source.json: unknown authority or usage');
}
const conceptDefaultByKey = new Map(conceptDefaults.map((row) => [row.conceptKey, row]));
const discoveryLinks = discoveryInput.links.map((link) => {
  const label = `discovery link ${link.linkId} ${link.conceptKey} ${link.piId}`;
  const concept = conceptByKey.get(link.conceptKey);
  if (!concept || concept.id !== link.conceptId) throw new Error(`${label}: unknown concept`);
  if (!mapperIds.has(link.piId)) throw new Error(`${label}: unknown Mapper ingredient`);
  if (conceptPiLinks.some((row) => row.conceptKey === link.conceptKey && row.piId === link.piId)) {
    throw new Error(`${label}: already linked in the frozen source`);
  }
  const decision = conceptDefaultByKey.get(link.conceptKey);
  const frozenOrder = decision
    ? [
        decision.defaultPiId,
        ...decision.alternativePiIds,
        ...Object.values(decision.scopes).flatMap((scope) => [
          scope.suggestedPiId,
          ...scope.eligiblePiIds,
        ]),
      ]
    : [];
  if (frozenOrder.includes(link.piId)) {
    throw new Error(`${label}: already a selectable candidate — discovery links never select`);
  }
  return {
    linkId: String(link.linkId),
    conceptId: String(link.conceptId),
    conceptKey: String(link.conceptKey),
    piId: String(link.piId),
    form: String(link.form),
    note: String(link.note),
  };
});
const discoveryData = {
  authority: discoveryInput.authority,
  usage: discoveryInput.usage,
  approvedOn: String(discoveryInput.approvedOn),
  approvedBy: String(discoveryInput.approvedBy),
  searchReleaseId: 'GELLATTI-SA10-2026-09-10-FINAL',
  source: {
    fileName: path.basename(CONCEPT_DISCOVERY_SOURCE),
    sha256: sha256(discoverySourceText),
  },
  counts: {
    links: discoveryLinks.length,
    concepts: new Set(discoveryLinks.map((link) => link.conceptKey)).size,
  },
  links: discoveryLinks,
};
const conceptDiscoverySource =
  `/** GENERATED by scripts/buildMapperSearchRuntimeRelease.mjs — do not edit. */\n` +
  `import type { MapperConceptDiscoveryData } from '../conceptDiscoveryTypes';\n\n` +
  `export const MAPPER_CONCEPT_DISCOVERY_LINKS: MapperConceptDiscoveryData = ${JSON.stringify(discoveryData)};\n`;
const conceptDiscoveryManifestSource =
  `/** GENERATED by scripts/buildMapperSearchRuntimeRelease.mjs — do not edit. */\n` +
  `export const MAPPER_CONCEPT_DISCOVERY_SOURCE_SHA256 = '${discoveryData.source.sha256}';\n` +
  `export const MAPPER_CONCEPT_DISCOVERY_SHA256 = '${sha256(conceptDiscoverySource)}';\n`;
...
const outputs = [
  ...
  [CONCEPT_DISCOVERY_OUT, conceptDiscoverySource],
  [CONCEPT_DISCOVERY_MANIFEST_OUT, conceptDiscoveryManifestSource],
];
